        ...
        g2.append(
            (
                ...
                [
                    ...
                    "U",
                    "-oU",
                    str(outdir / "urls_waymore.txt"),
                    "-oR",
                    str(outdir / "logs" / "waymore"),
                ],
                _maybe_timeout(1800),
            )
        )
            ...
            try:
                ...
                if netloc:
                    ...
                    if (
                        ...
                        and hostname not in existing_hosts
                        and (not domain or _is_under_domain(hostname, domain))
                    ):
                        new_hosts.add(
                            f"https://{netloc}" if parsed.scheme in ("http", "https") else u
                        )
            except Exception:
                pass
        if new_hosts:
            ...
            if t.has("httpx"):
                ...
                if httpx_out.exists() and read_lines(httpx_out):
                    ...
                    log(
                        "ok",
                        f"05-HARVEST: added {len(read_lines(targets_file)) - len(existing_hosts)} new target hosts",
                    )
...
async def phase_05b_APISPEC(
        ...
        async with _probe_sem:
            ...
            try:
                req = urllib.request.Request(
                    url, headers={"User-Agent": "Mozilla/5.0", **_ap_extra_headers}
                )
                ...
                if status in (200, 301, 302) and len(body) > 50:
                    if "swagger" in body.lower() or path.endswith("swagger.json"):
                    ...
                    elif "openapi" in body.lower() or path.endswith(
                        ...
                        "id_token" in body or "jwks_uri" in body or "authorization_endpoint" in body
                    ):
    ...
    tasks = [
        _probe_one(h.rstrip("/") if h.startswith("http") else f"https://{h.rstrip('/')}", p)
        for h in host_list
        for p in api_paths
    ]
