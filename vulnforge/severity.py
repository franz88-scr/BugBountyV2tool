...
from vulnforge.artifacts import ARTIFACTS, guess_severity
from vulnforge.utils import ensure, log, read_lines

# Patterns that indicate a phase produced no real findings.
...
def calculate_risk_score(outdir: Path) -> RiskScore:
    ...
    for art in ARTIFACTS:
        ...
        for line in read_lines(p):
            ...
            if not text:
                continue
            # Skip placeholder "no findings" messages — they are not real
            # vulnerabilities and should not inflate the risk score.
            if _NO_FINDINGS_RE.match(text):
                continue
    ...
    if total == 0:
        ...
        weighted_sum = sum(severity_counts[sev] * SEVERITY_WEIGHTS[sev] for sev in severity_counts)
    ...
    if severity_counts["critical"] > 0:
        recommendations.append(
            f"URGENT: {severity_counts['critical']} critical vulnerabilities require immediate remediation"
        )
    if severity_counts["high"] > 10:
        recommendations.append(
            f"High volume of high-severity findings ({severity_counts['high']}) — review attack surface"
        )
    if critical_paths > 0:
        recommendations.append(
            f"{critical_paths} critical exploit chains identified — prioritize chain breaking"
        )
    ...
    if total > 100:
        recommendations.append(
            "Large number of findings — consider targeted re-scan with --fast for quick wins"
        )
...
def write_risk_score(outdir: Path, risk: RiskScore) -> Path:
    ...
    out.write_text(json.dumps(risk.to_dict(), indent=2), encoding="utf-8")
    return out
