"""Interactive setup wizard for VulnForge CLI."""
...
from vulnforge.config import DOS_PHASES, PHASE_CATEGORIES, VALID_PHASES, WIZARD_PRESETS
from vulnforge.process import MAX_PARALLEL_JOBS
from vulnforge.utils import C, _auto_detect_proxy, _is_valid_hostname, log
...
def _box_line(content: str, width: int = 58) -> str:
    """Wrap content in a box line, padding to width."""
    visible_len = len(_strip_ansi(content))
    pad = max(0, width - visible_len)
    return f"{C['c']}║{C['r']}{content}{' ' * pad}{C['c']}║{C['r']}"


def _box_empty(width: int = 58) -> str:
    """Return an empty box line."""
    return f"{C['c']}║{' ' * width}║{C['r']}"
...
def _box_sep(width: int = 58) -> str:
    """Return a separator line for a box."""
    return f"{C['c']}╠{'═' * width}╣{C['r']}"
...
class InteractiveWizard:
    ...
    def run(self) -> argparse.Namespace:
        ...
        log(
            "info",
            f"{C['c']}Interactive setup wizard{C['r']} {C['d']}v2{C['r']} — press {C['y']}Ctrl+C{C['r']} anytime to abort\n",
        )
        self._profile_menu()
    ...
    def _profile_menu(self) -> None:
        ...
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}LOAD SAVED PROFILE{C['r']}"))
        print(_box_sep())

        if profiles:
            for i, p in enumerate(profiles, 1):
                preset_color = C["g"] if p["preset"] == "standard" else C["y"]
                print(_box_empty())
                print(
                    _box_line(
                        f"  {C['y']}{i}{C['r']}  {C['b']}{p['name']}{C['r']}  {preset_color}{p['preset']}{C['r']}  {C['c']}{p['phases']} phases{C['r']}"
                    )
                )
            print(_box_empty())
        else:
            print(_box_empty())
            print(_box_line(f"  {C['d']}No saved profiles found{C['r']}"))
            print(_box_empty())

        print(_box_sep())
        print(
            _box_line(
                f"  {C['g']}◆{C['r']} {C['y']}N{C['r']}  {C['b']}New scan{C['r']} (no profile)"
            )
        )
        print(_box_bot())
    ...
    def _preset_menu(self) -> None:
        ...
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}SELECT SCAN PRESET{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        ...
        for i, (key, preset) in enumerate(presets, 1):
            ...
            bar_len = 8
            filled = min(bar_len, int(phase_count / 20))
            phase_bar = f"{C['g']}{'█' * filled}{C['d']}{'░' * (bar_len - filled)}{C['r']}"
            print(
                _box_line(
                    f"  {C['y']}{i}{C['r']}  {C['b']}{preset['name']}{C['r']}  {phase_bar} {C['c']}{phase_count} phases{C['r']}"
                )
            )
            print(_box_line(f"      {C['d']}{preset['desc']}{C['r']}"))
            if i < len(presets):
                print(_box_empty())
    ...
    def _target_menu(self) -> None:
        domain_icon = f"{C['g']}●{C['r']}" if self.domain else f"{C['d']}○{C['r']}"
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}TARGET CONFIGURATION{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
                f"  {domain_icon} {C['b']}Domain:{C['r']}  {C['y']}{self.domain or 'not set'}{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_bot())
        ...
        domain = _prompt(
            ...
            validator=lambda v: (
                bool(v.strip())
                and all(_is_valid_hostname(d.strip()) for d in v.split(",") if d.strip())
            ),
            error_msg="Enter valid domain(s) with at least one dot each",
        )
        self.domain = domain
        self.out = _prompt("Output directory", default=f"./out_{self.domain.split(',')[0].strip()}")
    ...
    def _main_menu_loop(self) -> None:
        while True:
            ...
            choice = input(f"  {C['y']}▸{C['r']} {C['b']}Select section:{C['r']} ").strip().lower()
            if choice == "1":
                self._edit_target()
            elif choice == "2":
                self._edit_depth()
    ...
    def _print_main_menu(self) -> None:
        ...
        prof = f"  {C['d']}Profile: {self.profile_name}{C['r']}" if self.profile_name else ""
        ...
        # Status indicators
        domain_icon = f"{C['g']}●{C['r']}" if self.domain else f"{C['d']}○{C['r']}"
        phase_icon = f"{C['g']}●{C['r']}" if n > 0 else f"{C['d']}○{C['r']}"
        dos_icon = f"{C['red']}●{C['r']}" if self.config.get("dos_mode") else f"{C['d']}○{C['r']}"
        cookie_icon = f"{C['g']}●{C['r']}" if self.config.get("cookie") else f"{C['d']}○{C['r']}"

        # Phase progress bar
        phase_pct = int((n / total) * 100) if total > 0 else 0
        bar_len = 15
        filled = int(bar_len * phase_pct / 100)
        phase_bar = f"{C['g']}{'█' * filled}{C['d']}{'░' * (bar_len - filled)}{C['r']}"

        cookie_val = "set" if self.config.get("cookie") else "none"
        print()
        print(_box_top())
        print(
            _box_line(
                f"  {C['b']}VULNFORGE{C['r']}  {C['d']}Setup Wizard{C['r']}{prof}       Preset: {C['y']}{self.preset}{C['r']}"
            )
        )
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
                f"  {domain_icon} {C['y']}[1]{C['r']}  {C['b']}Target & Scope{C['r']}               {C['c']}{d}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {phase_icon} {C['y']}[2]{C['r']}  {C['b']}Scan Depth{C['r']}                   sqlmap={C['c']}{sqlmap}{C['r']}  delay={C['c']}{delay}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {phase_icon} {C['y']}[3]{C['r']}  {C['b']}Phase Selection{C['r']}              {phase_bar} {C['c']}{n}/{total}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {cookie_icon} {C['y']}[4]{C['r']}  {C['b']}Auth & Cookies{C['r']}               cookie={C['c']}{cookie_val}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['y']}[5]{C['r']}  {C['b']}Performance & Proxy{C['r']}          adaptive={C['c']}{adaptive}{C['r']}  proxy={C['c']}{prox}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {dos_icon} {C['y']}[6]{C['r']}  {C['b']}DoS & Rate Limits{C['r']}            dos={C['c']}{dos}{C['r']}  rate={C['c']}{rate} r/s{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['y']}[7]{C['r']}  {C['b']}Reporting{C['r']}                    {C['c']}{fmt}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['y']}[8]{C['r']}  {C['b']}Compliance & Audit{C['r']}           {C['c']}{compliance}{C['r']}  threat={C['c']}{threat}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['y']}[9]{C['r']}  {C['b']}Intelligence & ML{C['r']}            ml={C['c']}{ml}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['y']}[0]{C['r']}  {C['b']}Sampling{C['r']}                    mode={C['c']}{sample_mode}{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_sep())
        print(
            _box_line(
                f"  {C['g']}[S]{C['r']} Save    {C['g']}[L]{C['r']} Load    {C['g']}[R]{C['r']} Reset    {C['g']}[P]{C['r']} Preview"
            )
        )
        print(_box_empty())
        print(
            _box_line(
                f"  {C['g']}▸{C['r']} {C['g']}[Enter]{C['r']}  {C['b']}START SCAN{C['r']}                  {C['g']}[Q]{C['r']} Quit"
            )
        )
        print(_box_bot())
    ...
    def _edit_target(self) -> None:
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}TARGET & SCOPE{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['b']}Configure target domain(s) and output directory{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_bot())
        ...
        self.domain = _prompt(
            ...
            validator=lambda v: all(
                _is_valid_hostname(d.strip()) for d in v.split(",") if d.strip()
            ),
            error_msg="Enter valid domain(s) with at least one dot each",
        )
        self.out = _prompt("Output directory", default=f"./out_{self.domain.split(',')[0].strip()}")
    ...
    def _edit_depth(self) -> None:
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}SCAN DEPTH & TIMING{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['b']}Configure SQLmap level/risk and request delay{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_bot())

        self.config["sqlmap_level"] = int(
            _prompt(
    ...
    def _edit_phases(self) -> None:
        ...
        while True:
            ...
            print(
                _box_line(
                    f"  {C['b']}PHASE SELECTION{C['r']}  {phase_bar} {C['c']}{n}/{total} active{C['r']}  Preset: {C['y']}{self.preset}{C['r']}"
                )
            )
            print(_box_sep())
            print(_box_empty())
            ...
            for i, (cat_name, cat_data) in enumerate(cats, 1):
                ...
                cat_pct = int((active / len(phases)) * 100) if phases else 0
                cat_bar_len = 8
                cat_filled = int(cat_bar_len * cat_pct / 100)
                cat_bar = (
                    f"{C['g']}{'█' * cat_filled}{C['d']}{'░' * (cat_bar_len - cat_filled)}{C['r']}"
                )

                if active == len(phases):
                    color = C["g"]
                    icon = f"{C['g']}●{C['r']}"
                elif active > 0:
                    color = C["y"]
                ...
                print(
                    _box_line(
                        f"  {icon} {C['y']}{i}{C['r']}  {color}{cat_name}{C['r']}  {cat_bar} {C['c']}{active}/{len(phases)}{C['r']}"
                    )
                )
            ...
            print(
                _box_line(
                    f"  {C['g']}◆{C['r']} {C['g']}[a]{C['r']} Select all   {C['g']}◆{C['r']} {C['g']}[n]{C['r']} Deselect all   {C['g']}◆{C['r']} {C['g']}[s]{C['r']} Search"
                )
            )
            print(_box_empty())
            print(
                _box_line(
                    f"  {C['g']}▸{C['r']} {C['g']}[b]{C['r']}  {C['b']}Back to main menu{C['r']}"
                )
            )
            print(_box_bot())
    ...
    def _edit_category(self, cat_name: str, cat_data: Dict[str, Any]) -> None:
        ...
        while True:
            ...
            cat_bar = f"{C['g']}{'█' * filled}{C['d']}{'░' * (bar_len - filled)}{C['r']}"

            print()
            print(_box_top())
            print(
                _box_line(
                    f"  {C['b']}{cat_name}{C['r']}  {cat_bar} {C['c']}{active}/{len(phases)} active{C['r']}"
                )
            )
            print(_box_line(f"  {C['d']}{cat_data['desc']}{C['r']}"))
            print(_box_sep())
            print(_box_empty())
            ...
            for i, (pid, desc) in enumerate(phases, 1):
                ...
                print(
                    _box_line(
                        f"  {mark} {C['y']}{i:2}{C['r']}  {color}{pid}{C['r']}  {C['d']}{desc_display}{C['r']}"
                    )
                )
            ...
            print(
                _box_line(
                    f"  {C['g']}◆{C['r']} {C['g']}[a]{C['r']} Toggle all   {C['g']}◆{C['r']} {C['g']}[b]{C['r']}  {C['b']}Back{C['r']}"
                )
            )
            print(_box_bot())
    ...
    def _phase_search(self) -> None:
        ...
        matches = [
            (pid, desc)
            for pid, desc in _all_phase_list()
        ...
        print(
            _box_line(
                f"  {C['b']}SEARCH RESULTS{C['r']}  {C['c']}{len(matches)} matching phases{C['r']}"
            )
        )
        print(_box_sep())
        print(_box_empty())
        ...
        for i, (pid, desc) in enumerate(matches, 1):
            ...
            print(
                _box_line(
                    f"  {mark} {C['y']}{i:3}{C['r']}  {color}{pid}{C['r']}  {C['d']}{desc_display}{C['r']}"
                )
            )
        ...
        print(
            _box_line(
                f"  {C['g']}◆{C['r']} {C['g']}[a]{C['r']} Toggle all matches   {C['g']}◆{C['r']} {C['g']}[b]{C['r']}  {C['b']}Back{C['r']}"
            )
        )
        print(_box_bot())
    ...
    def _edit_auth(self) -> None:
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}AUTHENTICATION & COOKIES{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(_box_line(f"  {C['g']}●{C['r']} {C['b']}Configure cookies and HTTP headers{C['r']}"))
        print(_box_empty())
        print(_box_bot())
        ...
        self.config["extra_headers"] = (
            [h.strip() for h in extra_raw.split(",") if h.strip()] if extra_raw else []
        )
    ...
    def _edit_performance(self) -> None:
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}PERFORMANCE & PROXY{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['b']}Configure performance mode and proxy settings{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_sep())
        print(_box_line(f"  {C['b']}PERFORMANCE MODES:{C['r']}"))
        print(_box_empty())
        print(
            _box_line(
                f"  {C['g']}1{C['r']}  {C['b']}Safe mode{C['r']}       {C['d']}Conservative: start=1, max=4, max_procs=2{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}2{C['r']}  {C['b']}Balanced{C['r']}        {C['d']}Mid-tier: auto-scales between safe/aggressive{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}3{C['r']}  {C['b']}Aggressive{C['r']}      {C['d']}Max resources: all cores, minimal throttling{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_bot())
    ...
    def _edit_dos(self) -> None:
        dos_on = self.config.get("dos_mode", False)
        dos_icon = f"{C['red']}●{C['r']}" if dos_on else f"{C['d']}○{C['r']}"
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}DoS & RATE LIMITS{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
                f"  {dos_icon} {C['b']}DoS mode:{C['r']}  {C['y']}{'ON' if dos_on else 'OFF'}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}│{C['r']} {C['b']}Rate limit:{C['r']}  {C['y']}{self.config.get('rate_limit', 10)} req/s{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_bot())

        self.config["rate_limit"] = int(
            _prompt(
        ...
        if not self.config["dos_mode"]:
            print(
                f"  {C['d']}DoS phases disabled:{C['r']} 20-GRAPHQL, 23-RACE, 34-RATELIMIT, 38-SMUGGLE, 38b-H2SMUGGLE, 54-WS-FUZZ, 83-RACEBURST, 93-PWDSPRAY, 132-GQLABUSE, 136-RATELIMITBYPASS"
            )
    ...
    def _edit_report(self) -> None:
        ...
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}REPORTING & SAMPLING{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(f"  {C['g']}●{C['r']} {C['b']}Report format:{C['r']}  {C['y']}{fmt}{C['r']}")
        )
        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}Fuzz URLs:{C['r']}  {C['y']}{self.config.get('sample_urls_fuzz', '5')}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}Param URLs:{C['r']}  {C['y']}{self.config.get('sample_urls_params', '50')}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}└─{C['r']} {C['b']}Fast mode:{C['r']}  {C['y']}{'yes' if self.config.get('fast') else 'no'}{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_bot())
    ...
    def _edit_compliance(self) -> None:
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}COMPLIANCE & AUDIT{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
                f"  {C['d']}Generate compliance reports mapping findings to frameworks{C['r']}"
            )
        )
        print(_box_empty())
        print(
            _box_line(
                f"  {C['g']}1{C['r']}  {C['b']}PCI DSS v4.0{C['r']}     {C['d']}Payment card industry data security{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}2{C['r']}  {C['b']}HIPAA{C['r']}             {C['d']}Healthcare data protection{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}3{C['r']}  {C['b']}SOC 2 Type II{C['r']}     {C['d']}Service organization controls{C['r']}"
            )
        )
        print(_box_line(f"  {C['g']}4{C['r']}  {C['b']}All frameworks{C['r']}"))
        print(_box_line(f"  {C['g']}0{C['r']}  {C['d']}None (skip){C['r']}"))
        print(_box_empty())
        print(_box_bot())
    ...
    def _edit_threat_intel(self) -> None:
        ...
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}INTELLIGENCE & MACHINE LEARNING{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['b']}Threat Intel:{C['r']}  {C['y']}{threat}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}ML Phase Select:{C['r']}  {C['y']}{ml}{C['r']}"
            )
        )
        print(
            _box_line(f"  {C['d']}├─{C['r']} {C['b']}Cred Store:{C['r']}  {C['y']}{creds}{C['r']}")
        )
        print(
            _box_line(f"  {C['d']}└─{C['r']} {C['b']}Workspace:{C['r']}  {C['y']}{collab}{C['r']}")
        )
        print(_box_empty())
        print(_box_bot())
    ...
    def _edit_sample_size(self) -> None:
        ...
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}SAMPLING{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
                f"  {C['g']}1{C['r']}  {C['b']}Minimal{C['r']}  {C['d']}Test 1 item per tool/phase (fastest, least coverage){C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}2{C['r']}  {C['b']}Normal{C['r']}  {C['d']}Default sample sizes per tool (balanced){C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['g']}3{C['r']}  {C['b']}All{C['r']}  {C['d']}No sampling limits (slowest, full coverage){C['r']}"
            )
        )
        print(_box_empty())
        print(_box_line(f"  {C['d']}Current: {C['y']}{sample_mode}{C['r']}"))
        print(_box_empty())
        print(_box_bot())
    ...
    def _preview_scan(self) -> None:
        ...
        # Status indicators
        domain_icon = f"{C['g']}●{C['r']}" if self.domain else f"{C['red']}●{C['r']}"
        dos_icon = f"{C['red']}●{C['r']}" if self.config.get("dos_mode") else f"{C['d']}○{C['r']}"
        cookie_icon = f"{C['g']}●{C['r']}" if self.config.get("cookie") else f"{C['d']}○{C['r']}"
        ...
        print()
        print(_box_top())
        print(_box_line(f"  {C['b']}SCAN SUMMARY{C['r']}"))
        print(_box_sep())
        print(_box_empty())
        print(
            _box_line(
        ...
        print(
            _box_line(f"  {C['d']}├─{C['r']} {C['b']}Output:{C['r']}  {C['y']}{self.out}{C['r']}")
        )
        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}Preset:{C['r']}  {C['y']}{self.preset}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}Profile:{C['r']}  {C['y']}{self.profile_name or '(none)'}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}Proxy:{C['r']}  {C['y']}{proxy or 'auto-detected'}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}└─{C['r']} {C['b']}Phases:{C['r']}  {phase_bar} {C['c']}{n}/{total}{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_sep())
        print(_box_line(f"  {C['b']}PERFORMANCE{C['r']}"))
        print(_box_empty())

        if adaptive:
            print(
                _box_line(
                    f"  {C['g']}●{C['r']} {C['b']}Adaptive:{C['r']}  {C['y']}ON{C['r']} (start={self.config.get('adaptive_start')}, max={self.config.get('adaptive_max')})"
                )
            )
        else:
            print(
                _box_line(
                    f"  {C['d']}○{C['r']} {C['b']}Adaptive:{C['r']}  {C['d']}OFF{C['r']}  Max procs: {C['y']}{self.config.get('max_procs', 0) or 'auto'}{C['r']}"
                )
            )

        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}Rate limit:{C['r']}  {C['y']}{self.config.get('rate_limit', 0)} req/s{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}SQLmap:{C['r']}  {C['y']}{self.config.get('sqlmap_level')}/{self.config.get('sqlmap_risk')}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}└─{C['r']} {C['b']}Delay:{C['r']}  {C['y']}{self.config.get('delay')}s{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_sep())
        print(_box_line(f"  {C['b']}SECURITY & FEATURES{C['r']}"))
        print(_box_empty())
        print(
            _box_line(
                f"  {cookie_icon} {C['b']}Cookie:{C['r']}  {C['y']}{'set' if self.config.get('cookie') else 'none'}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {dos_icon} {C['b']}DoS mode:{C['r']}  {C['y']}{'yes' if self.config.get('dos_mode') else 'no'}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}Safe mode:{C['r']}  {C['y']}{'ON' if self.config.get('safe_mode') else 'OFF'}{C['r']}"
            )
        )
        print(
            _box_line(
        ...
        print(
            _box_line(
                f"  {C['d']}└─{C['r']} {C['b']}Report:{C['r']}  {C['y']}{self.config.get('report_format')}{C['r']}"
            )
        )
        print(_box_empty())
        print(_box_sep())
        print(_box_line(f"  {C['b']}INTELLIGENCE{C['r']}"))
        print(_box_empty())
        print(
            _box_line(
                f"  {C['g']}●{C['r']} {C['b']}Compliance:{C['r']}  {C['y']}{compliance}{C['r']}"
            )
        )
        print(
            _box_line(
                f"  {C['d']}├─{C['r']} {C['b']}Threat Intel:{C['r']}  {C['y']}{threat}{C['r']}"
            )
        )
        print(
            _box_line(
