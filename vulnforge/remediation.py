...
REMEDIATIONS: Dict[str, Remediation] = {
    ...
    "secrets": Remediation(
        cwe="CWE-798",
        title="Hardcoded Credentials / Secret Exposure",
        severity="high",
    ...
    "clickjacking": Remediation(
        ...
        references=[
            "https://cheatsheetseries.owasp.org/cheatsheets/Clickjacking_Prevention_Cheat_Sheet.html"
        ],
    ),
    ...
    "css_injection": Remediation(
        cwe="CWE-79",
        title="CSS Injection",
        severity="medium",
        description="User input is reflected in CSS context, enabling data exfiltration via CSS attribute selectors and background URLs.",
        remediation=(
            "1. Encode user input in CSS context using CSS encoding\n"
            "2. Use Content-Security-Policy with style-src restrictions\n"
            "3. Avoid rendering user input directly in style tags or style attributes\n"
            "4. Validate and sanitize input that appears in CSS selectors"
        ),
        references=["https://portswigger.net/web-security/css-injection"],
    ),
    "dangling_markup": Remediation(
        cwe="CWE-838",
        title="Dangling Markup Injection",
        severity="medium",
        description="User input is reflected in HTML without proper encoding, allowing unclosed tags to capture subsequent page content.",
        remediation=(
            "1. HTML-encode all user input before reflection in HTML context\n"
            "2. Use Content-Security-Policy to restrict form actions and resource loading\n"
            "3. Apply context-sensitive output encoding based on input placement"
        ),
        references=["https://portswigger.net/web-security/dangling-markup"],
    ),
    "mfa_bypass": Remediation(
        cwe="CWE-287",
        title="MFA/2FA Bypass",
        severity="high",
        description="Multi-factor authentication can be circumvented via fatigue attacks, backup code abuse, or enrollment bypass.",
        remediation=(
            "1. Implement rate limiting on MFA verification attempts\n"
            "2. Require user confirmation for MFA method enrollment\n"
            "3. Limit backup code generation and usage frequency\n"
            "4. Implement push notification throttling to prevent MFA fatigue\n"
            "5. Log and alert on rapid MFA attempts"
        ),
        references=["https://owasp.org/www-community/attacks/MFA_Bypass"],
    ),
    "redos": Remediation(
        cwe="CWE-1333",
        title="ReDoS - Regular Expression Denial of Service",
        severity="high",
        description="Application uses regex with catastrophic backtracking, enabling DoS with crafted inputs.",
        remediation=(
            "1. Review regex patterns for catastrophic backtracking (nested quantifiers)\n"
            "2. Set timeouts on regex execution\n"
            "3. Use simple string operations instead of complex regex\n"
    ...
    "cookie_toss": Remediation(
        ...
        severity="medium",
        description="Subdomains can overwrite cookies for the parent domain, enabling session fixation and CSRF-like attacks.",
        remediation=(
            "1. Use __Host- prefix for session cookies\n"
            "2. Set explicit Path attribute on cookies\n"
            "3. Avoid wildcard domain cookies\n"
            "4. Use Secure and HttpOnly flags on all sensitive cookies\n"
