"""
vulnforge.bot — Discord/Slack companion bot for live scan management.
...
Usage:
    # From CLI:
    vulnforge -d example.com --bot --bot-platform discord

    # Programmatic:
    from vulnforge.bot import start_bot
...
async def _run_discord_bot(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        def _safe_ensure_future(coro: Any) -> None:
        ...
        def on_finding(event: Any) -> None:
            ...
            now = time.time()
            with _bot_state._lock:
                last = _bot_state.last_post_time.get(domain, 0)
                buf = _bot_state.findings_buffer[domain]
                buf.append(finding)
                if len(buf) > MAX_BUF:
                    buf[:] = buf[-MAX_BUF:]
                ...
                if now - last >= 5 and _bot_state.findings_buffer[domain]:
                    ...
                    dropped = len(buffered) - 3
                    if dropped > 0:
                        log(
                            "warn",
                            f"bot: {dropped} finding(s) dropped for {domain} (throttle limit)",
                        )
                    for f in buffered[-3:]:  # Max 3 per batch
                        _safe_ensure_future(send_finding(f))

        def on_phase_complete(event: Any) -> None:
            phase = event.data.get("phase", "")
            elapsed = event.data.get("elapsed", 0)
            msg = f"\u2705 Phase `{phase}` completed in {elapsed:.1f}s"
            _safe_ensure_future(send_message(msg))
        ...
        def on_scan_complete(event: Any) -> None:
            ...
            embed = {
                ...
                "fields": [
                    ...
                    {
                        "name": "Total Findings",
                        "value": str(data.get("total_findings", 0)),
                        "inline": True,
                    },
                ],
                "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
            ...
            _safe_ensure_future(send_message("", embed))
        ...
        async with session.ws_connect(f"{ws_url}?v=10&encoding=json") as ws:
            ...
            async def _heartbeat_loop():
                nonlocal last_heartbeat, seq
                try:
                    while True:
                        await asyncio.sleep(heartbeat_interval / 1000 * 0.75)
                        now = time.time() * 1000
                            ...
                            if event_type == "MESSAGE_CREATE":
                                ...
                                if cmd == "!status":
                                    ...
                                    if stats:
                                        ...
                                        if fields:
                                            embed["fields"] = fields  # type: ignore[assignment]
                                    ...
                                    findings = _bot_state.findings_buffer.get(
                                        ...
                                        if domain != "latest"
                                            ...
                                            list(_bot_state.findings_buffer.keys())[-1]
                                            if _bot_state.findings_buffer
                                            else ""
                                        ),
                                        [],
                                    )
                                    if sev_filter:
                                        findings = [
                                            f for f in findings if f.get("severity") == sev_filter
                                        ]
                                    if not findings:
                                        ...
                                        for f in findings[-10:]:
                                            ...
                                            lines.append(
                                                f"{emoji} `{f.get('phase', '?')}` {f.get('text', '')[:100]}"
                                            )
                                        await send_message("\n".join(lines))
...
async def _run_slack_bot(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        def on_finding(event: Any) -> None:
            ...
            source = event.data.get("source", "")
            mention = (
                "<!channel>" if severity in ("critical", "high") and mention_on_critical else ""
            )
