...
def _validate_webhook_url_realtime(url: str) -> bool:
    """Re-validate webhook URL immediately before sending (defense against DNS rebinding)."""
...
def send_scan_summary(
    ...
    *,
    ...
    if notify_url:
        ...
        if hostname.endswith(".slack.com") or hostname == "slack.com":
            return _send_slack(notify_url, message)
        elif hostname.endswith(".discord.com") or hostname == "discord.com":
            return _send_discord(notify_url, message)
        elif hostname.endswith(".api.telegram.org") or hostname == "api.telegram.org":
            ...
            if bot_token and chat_id:
                ...
                log(
                    "warn",
                    "Telegram URL must include bot token and chat_id: https://api.telegram.org/botTOKEN/sendMessage?chat_id=CHAT_ID",
                )
                return False
...
def _send_slack(webhook_url: str, text: str) -> bool:
    ...
    if not _validate_webhook_url_realtime(webhook_url):
        return False
    try:
...
_WARNED_TELEGRAM = False


def _send_telegram(bot_token: str, chat_id: str, text: str) -> bool:
    """Send a Telegram bot notification."""
    global _WARNED_TELEGRAM
    if not _WARNED_TELEGRAM:
        log(
            "warn",
            "Telegram bot token is sent in the URL path and may be visible in proxy logs",
        )
        _WARNED_TELEGRAM = True
    if not chat_id:
        return False
    try:
        url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
        if not _validate_webhook_url_realtime(url):
            return False
        ...
        try:
            ...
            return data.get("ok", False)  # type: ignore[no-any-return]
        finally:
            resp.close()
    except Exception as exc:
