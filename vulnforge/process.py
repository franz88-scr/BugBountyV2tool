...
from vulnforge.config import _HOSTNAME_RE, DISCOVERY_PHASES, VALID_PHASES, PipelineConfig
from vulnforge.utils import _patch_socks, _set_proxy_env, _unpatch_socks, ensure, log
...
_CANCEL_TIMEOUT_EVENT = threading.Event()
...
from vulnforge.resource_monitor import AdaptiveThreadSemaphore  # noqa: E402
...
def reset_globals() -> None:
    ...
    from vulnforge.resource_monitor import reset_resource_monitor
    ...
    from vulnforge.utils import _unpatch_socks
...
async def _kill_child_async(proc: Any) -> None:
    try:
...
def _needs_proxychains(cmd: List[str], *, proxychains: Optional[bool] = None) -> bool:
    ...
    # except DNS tool wrappers and specific tool runners that have native proxy support
    # or don't work well with proxychains — proxychains slows DNS queries unnecessarily.
    if cmd[0] == "bash" and isinstance(cmd[1], str) and cmd[1].endswith((".sh", ".bash")):
        script_name = Path(cmd[1]).stem.lower()
        for _t in _DNS_TOOLS:
            if (
                _t == script_name
                or script_name.startswith(_t + ".")
                or script_name.startswith(_t + "-")
            ):
                return False
        # Exclude specific tool runners that handle proxy natively or break with proxychains
        if script_name in (
            "playwright_xss_runner",
            "sqlmap_11b_runner",
            "sqlmap_11b_deep_runner",
        ):
            return False
        return True
    return False
...
def _run_blocking(
    ...
    global _OS_PROC_ACTIVE, _NEXT_CANCEL_ID
    cmd = _proxify_cmd(cmd)

    # Dry-run mode: print command and skip execution
    if os.environ.get("VULNFORGE_DRY_RUN") == "1":
    ...
    # Verify log paths are not symlink attacks before writing
    for _p in (log_path, err_path):
        if _p.is_symlink():
            _real = _p.resolve()
            try:
                _real.relative_to(_p.parent.resolve())
            except ValueError:
                log("WARNING", f"symlink escape detected in log path: {_p} → {_real}; removing")
                _p.unlink()

    # Build subprocess env: always snapshot from os.environ + proxy state to
    # avoid race conditions when _push_phase_proxy mutates os.environ globally.
    snap = get_proxy_state()
    with _ENV_LOCK:
        if _bypass_proxy(cmd):
            # DNS tools bypass proxy entirely
    ...
    with _CANCEL_EVENTS_LOCK:
        cancel_id = _NEXT_CANCEL_ID
        _NEXT_CANCEL_ID += 1
        _CANCEL_EVENTS[cancel_id] = cancel_event
    if not _OS_PROC_SEM.acquire(blocking=True, timeout=120):
        with _CANCEL_EVENTS_LOCK:
            _CANCEL_EVENTS.pop(cancel_id, None)
        log("WARNING", f"_OS_PROC_SEM acquire timed out after 120s, skipping: {cmd[0]}")
    ...
    try:
        ...
        try:
            _log_fd = os.open(
                log_path, os.O_WRONLY | os.O_CREAT | os.O_TRUNC | os.O_NOFOLLOW, 0o644
            )
        except OSError:
            log("WARNING", f"cannot open log path {log_path}; logging to /dev/null")
            _log_fd = os.open(os.devnull, os.O_WRONLY)
        try:
            _err_fd = os.open(
                err_path, os.O_WRONLY | os.O_CREAT | os.O_TRUNC | os.O_NOFOLLOW, 0o644
            )
        except OSError:
            log("WARNING", f"cannot open stderr log path {err_path}; logging to /dev/null")
            _err_fd = os.open(os.devnull, os.O_WRONLY)
        with os.fdopen(_log_fd, "wb") as logf, os.fdopen(_err_fd, "wb") as errf:
            try:
                ...
                _register_proc(proc)
                if not _wait_proc(proc, timeout, cancel_event):
                    _kill_proc(proc)
                    _timed_out = True
                else:
                    _result_rc = proc.returncode
        ...
        return _result_rc, time.monotonic() - t0
    finally:
        with _CANCEL_EVENTS_LOCK:
            _CANCEL_EVENTS.pop(cancel_id, None)
        with _OS_PROC_ACTIVE_LOCK:
...
async def _run(
    ...
    with _CIRCUIT_BREAKER_LOCK:
        if tool_name and tool_name in _CIRCUIT_BREAKER_OPEN:
            if not quiet:
                log(
                    "warn",
                    f"{name}: skipping {tool_name} (circuit breaker open after {_CIRCUIT_BREAKER_FAILURES.get(tool_name, 0)} failures)",
                )
            return StepResult(
                name, cmd, 0, 0.0, outdir / "logs" / f"{name}.log", note=note or "circuit breaker"
            )
    ...
    if not quiet:
        ...
        _VALUE_FLAGS = {"-H", "--header", "-u", "--cookie", "-b", "--headers"}
        _log_args: List[str] = []
        _redact_next = False
        for a in cmd:
            if _redact_next:
                ...
                _log_args.append(a)
                _redact_next = True
            elif any(kw in a.lower() for kw in _SENSITIVE_KW):
                _log_args.append("***")
            else:
                _log_args.append(a)
        ...
        if tool_name:
            with _CIRCUIT_BREAKER_LOCK:
                if rc not in (0, 1, 2, None):
                    _CIRCUIT_BREAKER_FAILURES[tool_name] = (
                        _CIRCUIT_BREAKER_FAILURES.get(tool_name, 0) + 1
                    )
                    if _CIRCUIT_BREAKER_FAILURES[tool_name] >= _CIRCUIT_BREAKER_THRESHOLD:
                        _CIRCUIT_BREAKER_OPEN.add(tool_name)
                        log(
                            "warn",
                            f"Circuit breaker OPEN for {tool_name}: {_CIRCUIT_BREAKER_FAILURES[tool_name]} consecutive failures — skipping for rest of scan",
                        )
                else:
    ...
    if sem is not None:
        async with sem:
            ...
            if max_retries > 0 and result.rc not in (0, 1, 2, None, 124, 127) and result.rc > 0:
...
def _wait_proc(
    proc: subprocess.Popen, timeout: int, cancel_event: Optional[threading.Event] = None
) -> bool:
    ...
    while time.monotonic() < deadline:
        if cancel_event.is_set() or _CANCEL_TIMEOUT_EVENT.is_set():
            return False
        try:
            proc.wait(timeout=1)
            return True
...
def cancel_task(cancel_id: int) -> None:
    with _CANCEL_EVENTS_LOCK:
        event = _CANCEL_EVENTS.get(cancel_id)
        ...
        try:
            if phase_proxy and not is_discovery:
                ...
                _USE_PROXYCHAINS = bool(
                    shutil.which("proxychains4") and phase_proxy.startswith("socks")
                )
            ...
            _PROXY_STATE_STACK.pop()
            raise


async def _pop_phase_proxy(name: str = "") -> None:
    """Restore the proxy state saved by _push_phase_proxy."""
    global _USE_PROXYCHAINS, _PIPELINE_CFG, _PROXY_TIMEOUT_MULTIPLIER, _PROXY_STATE_STACK
    lock = _get_proxy_lock()
    async with lock:
        if not _PROXY_STATE_STACK:
            log("WARNING", f"_pop_phase_proxy({name}): stack empty, nothing to pop")
            return
        saved = _PROXY_STATE_STACK[-1]
        if name and saved.get("_phase") and saved["_phase"] != name:
            log(
                "warn",
                f"_pop_phase_proxy({name}): top of stack is for phase {saved['_phase']!r}, forcing pop anyway",
            )
        _PROXY_STATE_STACK.pop()
        _unpatch_socks()
        ...
        for v, val in saved["_env"].items():
            with _ENV_LOCK:
                if val is not None:
                    ...
                    os.environ.pop(v, None)
        if _PIPELINE_CFG.proxy and _PIPELINE_CFG.proxy.startswith(
...
async def run_parallel(
    jobs: List[Tuple[str, List[str], int]], outdir: Path, desc: str = "jobs", quiet: bool = False
) -> List[StepResult]:
...
async def _update_nuclei_templates(outdir: Path, proxy: str = "") -> None:
    ...
    if cache_stamp.exists():
        try:
            age = time.time() - float(
                cache_stamp.read_text(encoding="utf-8", errors="ignore").strip()
            )
            if age < 86400:
                return
        except (ValueError, OSError):
    ...
    if not await asyncio.to_thread(_OS_PROC_SEM.acquire, blocking=True, timeout=120):
        log("WARNING", "nuclei template update: OS process semaphore timed out")
        return
    try:
        ...
        if rc == 0:
            ...
            log("WARNING", f"nuclei -update-templates returned {rc}")
    finally:
        _OS_PROC_SEM.release()
...
def _atomic_write_json(path: Path, payload: dict) -> None:
    ...
    if path.is_symlink():
        ...
        if not real.is_relative_to(path.parent.resolve()):
            log("WARNING", f"atomic_write_json: removing symlink escape: {path} → {real}")
            path.unlink()
    fd, tmp_path = tempfile.mkstemp(dir=path.parent, suffix=".tmp")
    try:
        with os.fdopen(fd, "w") as f:
            json.dump(payload, f, indent=2, default=str)
        # TOCTOU-safe: verify destination is not a symlink before renaming
        try:
            _target_fd = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_NOFOLLOW)
            os.close(_target_fd)
        except OSError:
            log("WARNING", f"atomic_write_json: refusing to replace symlink target: {path}")
            raise
...
class AsyncJobScheduler:
    ...
    def _ensure_semaphore(self) -> Any:
        if self._semaphore is None:
            try:
                from vulnforge.resource_monitor import AdaptiveSemaphore

                self._semaphore = AdaptiveSemaphore(self._max_concurrent)
            except ImportError:
    ...
    def _check_memory_pressure(self) -> None:
        ...
        try:
            ...
            if mem.percent > 85 and not self._memory_pressure:
                ...
                log(
                    "warn",
                    f"AsyncJobScheduler: memory pressure ({mem.percent}%), "
                ...
                log(
                    "info",
                    f"AsyncJobScheduler: memory recovered ({mem.percent}%), "
                    f"concurrency restored to {self._max_concurrent}",
                )
                if hasattr(self._semaphore, "resize"):
                    self._semaphore.resize(self._max_concurrent)
        except ImportError:
            pass
