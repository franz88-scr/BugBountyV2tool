"""ML-based vulnerability detection and classification for VulnForge.

Uses pattern matching, heuristic scoring, and statistical classification
to identify, rank, and categorize security vulnerabilities from scan output.

No external ML frameworks required — implements lightweight classifiers
using Python stdlib with optional scikit-learn acceleration.

Usage:
    from vulnforge.ml_vuln import classify_findings, VulnerabilityClassifier
    classifier = VulnerabilityClassifier()
    classified = classifier.classify(outdir)
"""

from __future__ import annotations

import json
import re
from collections import Counter, defaultdict
from dataclasses import dataclass, field
from pathlib import Path
from typing import Any, Dict, List, Optional, Tuple

from vulnforge.artifacts import ARTIFACTS
from vulnforge.utils import ensure, log, read_lines

# ── Vulnerability signature database ────────────────────────────────

VULN_SIGNATURES: List[Dict[str, Any]] = [
    {
        "pattern": r"(?i)sql\s*(syntax|error|injection|blind)|mysql_fetch|ORA-\d{5}",
        "category": "sqli",
        "severity": "critical",
        "cwe": "CWE-89",
        "confidence": 0.95,
    },
    {
        "pattern": r"(?i)<script[^>]*>|alert\(|document\.cookie|onerror=|onload=",
        "category": "xss",
        "severity": "high",
        "cwe": "CWE-79",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)169\.254\.169\.254|aws.*metadata|imds|cloud\.metadata",
        "category": "ssrf",
        "severity": "critical",
        "cwe": "CWE-918",
        "confidence": 0.95,
    },
    {
        "pattern": r"(?i)/etc/passwd|/etc/shadow|\.\.\/\.\.\/|path\s*traversal",
        "category": "lfi",
        "severity": "high",
        "cwe": "CWE-22",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)default\s*(password|cred|login)|admin:admin|root:toor",
        "category": "default_creds",
        "severity": "critical",
        "cwe": "CWE-798",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)api[_-]?key\s*[:=]\s*['\"][A-Za-z0-9]{20,}",
        "category": "secrets",
        "severity": "high",
        "cwe": "CWE-798",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)(AKIA[0-9A-Z]{16}|ghp_[A-Za-z0-9]{36}|sk-[A-Za-z0-9]{20,})",
        "category": "secrets",
        "severity": "critical",
        "cwe": "CWE-798",
        "confidence": 0.95,
    },
    {
        "pattern": r"(?i)CORS.*(?:origin|Access-Control-Allow-Origin).*(?:\*|null)",
        "category": "cors",
        "severity": "medium",
        "cwe": "CWE-942",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)open\s*redirect|redirect_uri.*(?:\.\.\/|evil\.com)",
        "category": "open_redirect",
        "severity": "medium",
        "cwe": "CWE-601",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)SSRF|server.side.request",
        "category": "ssrf",
        "severity": "high",
        "cwe": "CWE-918",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)command\s*injection|;\s*(?:ls|cat|id|whoami)|`[^`]+`|system\(",
        "category": "cmdi",
        "severity": "critical",
        "cwe": "CWE-78",
        "confidence": 0.95,
    },
    {
        "pattern": r"(?i)deserialization|pickle\.loads|yaml\.load\(|Marshal\.Deserialize",
        "category": "deserialization",
        "severity": "critical",
        "cwe": "CWE-502",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)template.*inject|SSTI|\{\{.*\}\}|<%=.*%>",
        "category": "ssti",
        "severity": "critical",
        "cwe": "CWE-1336",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)XML\s*entity|XXE|DOCTYPE|ENTITY\s+%",
        "category": "xxe",
        "severity": "high",
        "cwe": "CWE-611",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)CSRF|cross.site.request.forgery|missing.*token",
        "category": "csrf",
        "severity": "medium",
        "cwe": "CWE-352",
        "confidence": 0.75,
    },
    {
        "pattern": r"(?i)insecure\s*cookie|missing.*[Ss]ecure|httponly.*false|samesite.*none",
        "category": "cookie",
        "severity": "medium",
        "cwe": "CWE-614",
        "confidence": 0.70,
    },
    {
        "pattern": r"(?i)missing.*(?:X-Frame-Options|Content-Security-Policy|Strict-Transport-Security)",
        "category": "header",
        "severity": "low",
        "cwe": "CWE-693",
        "confidence": 0.60,
    },
    {
        "pattern": r"(?i)JWT.*(?:none|weak|HS256.*RS256|algorithm\s*confusion)",
        "category": "jwt",
        "severity": "high",
        "cwe": "CWE-327",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)IDOR|insecure\s*direct|object\s*reference|/\d+/edit",
        "category": "idor",
        "severity": "high",
        "cwe": "CWE-639",
        "confidence": 0.75,
    },
    {
        "pattern": r"(?i)CRLF|\\r\\n|header\s*inject|response\s*splitting",
        "category": "crlf",
        "severity": "medium",
        "cwe": "CWE-113",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)file\s*upload|unrestricted.*upload|webshell|\.php.*upload",
        "category": "upload",
        "severity": "high",
        "cwe": "CWE-434",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)information\s*disclosure|stack\s*trace|debug\s*mode|verbose\s*error",
        "category": "info_disclosure",
        "severity": "low",
        "cwe": "CWE-200",
        "confidence": 0.70,
    },
    {
        "pattern": r"(?i)rate\s*limit|brute\s*force|lockout|account\s*enum",
        "category": "auth",
        "severity": "medium",
        "cwe": "CWE-307",
        "confidence": 0.65,
    },
    {
        "pattern": r"(?i)exposed.*(?:database|admin|panel|console)|phpMyAdmin|Adminer",
        "category": "exposed_databases",
        "severity": "critical",
        "cwe": "CWE-200",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)subdomain\s*takeover|CNAME.*(?:s3|github\.io|azure|herokuapp)",
        "category": "takeover",
        "severity": "high",
        "cwe": "CWE-440",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)host.header\s*inject|X-Forwarded-Host|X-Original-URL",
        "category": "host_header",
        "severity": "medium",
        "cwe": "CWE-644",
        "confidence": 0.75,
    },
    {
        "pattern": r"(?i)SSRF.*(?:metadata|169\.254|imds|cloud-metadata)|metadata.*SSRF",
        "category": "ssrf_cloud",
        "severity": "critical",
        "cwe": "CWE-918",
        "confidence": 0.95,
    },
    {
        "pattern": r"(?i)JWT.*(?:none|alg\s*=\s*none|algorithm\s*none)",
        "category": "jwt_none",
        "severity": "critical",
        "cwe": "CWE-327",
        "confidence": 0.95,
    },
    {
        "pattern": r"(?i)JWT.*(?:weak|secret|hardcoded|known\s*key)",
        "category": "jwt_weak",
        "severity": "high",
        "cwe": "CWE-798",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)SSTI.*(?:evaluated|code\s*execution|template\s*inject)|template.*(?:code|exec|system)",
        "category": "ssti_rce",
        "severity": "critical",
        "cwe": "CWE-1336",
        "confidence": 0.95,
    },
    {
        "pattern": r"(?i)SSRF.*(?:internal|127\.0\.0\.1|localhost|0\.0\.0\.0|metadata)",
        "category": "ssrf_internal",
        "severity": "critical",
        "cwe": "CWE-918",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)path\s*traversal.*(?:etc/passwd|win\.ini|proc/)|LFI.*(?:confirmed|file\s*read)",
        "category": "lfi_confirmed",
        "severity": "critical",
        "cwe": "CWE-22",
        "confidence": 0.95,
    },
    {
        "pattern": r"(?i)SQL\s*injection.*(?:blind|time.based|out.of.band)|blind.*SQL.*inject",
        "category": "sqli_blind",
        "severity": "critical",
        "cwe": "CWE-89",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)RCE|remote\s*code\s*execution|command\s*inject.*(?:confirmed|output)",
        "category": "rce",
        "severity": "critical",
        "cwe": "CWE-78",
        "confidence": 0.95,
    },
    {
        "pattern": r"(?i)SSRF.*(?:OOB|out.of.band|callback|interactsh|oast)",
        "category": "ssrf_oob",
        "severity": "critical",
        "cwe": "CWE-918",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)KID.*(?:path.*traversal|traversal|etc/passwd)|kid.*inject",
        "category": "jwt_kid",
        "severity": "high",
        "cwe": "CWE-327",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)JKU.*(?:SSRF|attacker|http://)|x5u.*(?:SSRF|attacker)",
        "category": "jwt_jku",
        "severity": "high",
        "cwe": "CWE-327",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)IDOR.*(?:confirmed|direct|sequential|modify)|Insecure.*Direct.*Reference",
        "category": "idor_confirmed",
        "severity": "high",
        "cwe": "CWE-639",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)mass\s*assignment|privilege\s*escalation.*(?:role|admin)|horizontal.*escalat",
        "category": "priv_esc",
        "severity": "critical",
        "cwe": "CWE-269",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)file\s*upload.*(?:polyglot|bypass|webshell|unrestricted)|upload.*(?:webshell|PHP|ASP)",
        "category": "upload_rce",
        "severity": "critical",
        "cwe": "CWE-434",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)S3\s*bucket.*(?:public|listing|write|upload)|AWS.*bucket.*(?:open|access)",
        "category": "s3_misconfig",
        "severity": "high",
        "cwe": "CWE-538",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)GCP.*bucket.*(?:public|allUsers|allAuthenticatedUsers)|storage.*ACL.*public",
        "category": "gcp_misconfig",
        "severity": "high",
        "cwe": "CWE-538",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)Azure.*blob.*(?:public|anonymous|open)|blob.*container.*(?:list|read|write)",
        "category": "azure_misconfig",
        "severity": "high",
        "cwe": "CWE-538",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)Docker\s*API.*(?:exposed|unauthenticated|remote)|container.*(?:escape|breakout)",
        "category": "docker_escape",
        "severity": "critical",
        "cwe": "CWE-250",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)K8s.*(?:unauth|API.*exposed|RBAC.*bypass)|kubernetes.*(?:secret|token|serviceaccount)",
        "category": "k8s_rbac",
        "severity": "critical",
        "cwe": "CWE-269",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)GraphQL.*(?:introspect|mutation|batch|dos)|graphiql.*(?:exposed|playground)",
        "category": "graphql_abuse",
        "severity": "high",
        "cwe": "CWE-200",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)CORS.*(?:wildcard|\*|null.*origin|reflect)|Access-Control-Allow-Origin.*\*",
        "category": "cors_wildcard",
        "severity": "medium",
        "cwe": "CWE-942",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)cookie.*(?:missing.*httponly|missing.*secure|samesite.*none|session.*fixation)",
        "category": "cookie_misconfig",
        "severity": "medium",
        "cwe": "CWE-614",
        "confidence": 0.75,
    },
    {
        "pattern": r"(?i)host.header.*(?:inject|spoof|manipulat)|X-Forwarded.*(?:For|Host).*inject",
        "category": "host_header_inject",
        "severity": "medium",
        "cwe": "CWE-644",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)HTTP.*request.*smuggl|CL.TE|TE.CL|TE.TE|Transfer-Encoding.*chunked",
        "category": "smuggling",
        "severity": "critical",
        "cwe": "CWE-444",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)CRLF.*inject|\\r\\n.*inject|header.*inject|response.*split",
        "category": "crlf_inject",
        "severity": "medium",
        "cwe": "CWE-113",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)prototype.*pollution|__proto__.*admin|constructor.*prototype",
        "category": "proto_pollution",
        "severity": "high",
        "cwe": "CWE-1321",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)nosql.*inject|\$gt|\$ne|\$where|\$regex.*mongo",
        "category": "nosqli",
        "severity": "high",
        "cwe": "CWE-943",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)LDAP.*inject|ldap.*error|javax\.naming|invalid.*dn\s*syntax",
        "category": "ldap_inject",
        "severity": "high",
        "cwe": "CWE-90",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)xpath.*inject|xpath.*error|system\.xml\.xpath|XPathException",
        "category": "xpath_inject",
        "severity": "high",
        "cwe": "CWE-91",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)log.*poison|log.*inject|CRLF.*log|log.*inclusion",
        "category": "log_poison",
        "severity": "high",
        "cwe": "CWE-117",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)subdomain.*takeover|dangling.*CNAME|CNAME.*(?:s3|github|azure|heroku|fastly)",
        "category": "subdomain_takeover",
        "severity": "high",
        "cwe": "CWE-440",
        "confidence": 0.85,
    },
    {
        "pattern": r"(?i)open\s*redirect.*(?:confirmed|parameter|302.*Location)|redirect.*(?:evil|attacker|malicious)",
        "category": "open_redirect_confirmed",
        "severity": "medium",
        "cwe": "CWE-601",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)cache.*poison|cache.*deception|X-Forwarded.*Host.*cache|Vary.*cache.*bypass",
        "category": "cache_poison",
        "severity": "high",
        "cwe": "CWE-444",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)IDOR.*(?:sequential|UUID|guid|direct)|Insecure.*Direct.*Object.*Reference",
        "category": "idor",
        "severity": "high",
        "cwe": "CWE-639",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)DNS.*zone.*transfer.*(?:succeed|vulnerable|AXFR)|zone.*transfer.*(?:found|records)",
        "category": "dns_zt",
        "severity": "medium",
        "cwe": "CWE-200",
        "confidence": 0.90,
    },
    {
        "pattern": r"(?i)password.*(?:spray|spraying|brute)|credential.*stuffing|rate.*limit.*bypass",
        "category": "auth_attack",
        "severity": "high",
        "cwe": "CWE-307",
        "confidence": 0.80,
    },
    {
        "pattern": r"(?i)SSRF.*(?:webhook|callback|OAST|interactsh)|server.side.*request.*forgery.*OOB",
        "category": "ssrf_webhook",
        "severity": "critical",
        "cwe": "CWE-918",
        "confidence": 0.90,
    },
]

SEVERITY_ORDER = {"critical": 0, "high": 1, "medium": 2, "low": 3, "info": 4}


@dataclass
class ClassifiedVulnerability:
    """A classified vulnerability finding."""

    text: str
    category: str
    severity: str
    cwe: str
    confidence: float
    source_file: str
    phase: str
    host: str = ""
    url: str = ""
    matched_patterns: List[str] = field(default_factory=list)
    risk_score: float = 0.0

    def to_dict(self) -> Dict[str, Any]:
        return {
            "text": self.text,
            "category": self.category,
            "severity": self.severity,
            "cwe": self.cwe,
            "confidence": round(self.confidence, 3),
            "source_file": self.source_file,
            "phase": self.phase,
            "host": self.host,
            "url": self.url,
            "risk_score": round(self.risk_score, 3),
        }


class VulnerabilityClassifier:
    """Pattern-based vulnerability classifier with Bayesian confidence adjustment."""

    def __init__(self) -> None:
        self._patterns: List[Tuple[re.Pattern, Dict[str, Any]]] = []
        for sig in VULN_SIGNATURES:
            compiled = re.compile(sig["pattern"])
            self._patterns.append((compiled, sig))
        self._global_counts: Counter = Counter()
        self._phase_counts: Dict[str, Counter] = defaultdict(Counter)

    def _match_finding(self, text: str) -> List[Tuple[Dict[str, Any], float, int]]:
        """Match a finding text against all vulnerability signatures."""
        matches = []
        for idx, (compiled, sig) in enumerate(self._patterns):
            m = compiled.search(text)
            if m:
                groups = [g for g in m.groups() if g]
                pattern_boost = min(0.1, len(groups) * 0.02)
                effective_confidence = min(1.0, sig["confidence"] + pattern_boost)
                matches.append((sig, effective_confidence, idx))
        return matches

    def _compute_risk_score(self, severity: str, confidence: float, host_findings: int) -> float:
        """Compute a 0-1 risk score combining severity, confidence, and host density."""
        sev_score = {"critical": 1.0, "high": 0.8, "medium": 0.5, "low": 0.25, "info": 0.1}
        base = sev_score.get(severity, 0.5)
        density_factor = min(1.0, host_findings / 20.0)
        return base * confidence * (0.7 + 0.3 * density_factor)

    def classify(
        self,
        outdir: Path,
        *,
        host_filter: Optional[str] = None,
        min_confidence: float = 0.5,
    ) -> List[ClassifiedVulnerability]:
        """Classify all vulnerability findings from scan output.

        Args:
            outdir: Output directory containing scan artifacts.
            host_filter: Only classify findings matching this host.
            min_confidence: Minimum confidence threshold (0-1).

        Returns:
            List of ClassifiedVulnerability sorted by risk_score descending.
        """
        all_findings: List[str] = []
        finding_sources: List[Tuple[str, str]] = []

        for art in ARTIFACTS:
            if not art.vuln_type:
                continue
            fpath = outdir / art.filename
            if not fpath.exists():
                continue
            for line in read_lines(fpath):
                text = line.strip()
                if text:
                    all_findings.append(text)
                    finding_sources.append((art.filename, art.phase))

        # Classify each finding
        classified: List[ClassifiedVulnerability] = []
        host_finding_counts: Counter = Counter()

        for i, text in enumerate(all_findings):
            matches = self._match_finding(text)
            if not matches:
                continue

            # Extract host from URL-like findings
            host = ""
            url = ""
            if text.startswith("http"):
                try:
                    from urllib.parse import urlparse

                    parsed = urlparse(text)
                    host = parsed.hostname or ""
                    url = text
                except Exception:
                    pass

            if host_filter and host != host_filter:
                continue

            # Take the highest-confidence match (tiebreak: later/specific signature index wins)
            matches.sort(key=lambda x: (x[1], x[2]), reverse=True)
            best_sig, best_confidence, _ = matches[0]

            if best_confidence < min_confidence:
                continue

            # Track host density
            if host:
                host_finding_counts[host] += 1

            source_file, phase = finding_sources[i] if i < len(finding_sources) else ("", "")

            classified.append(
                ClassifiedVulnerability(
                    text=text,
                    category=best_sig["category"],
                    severity=best_sig["severity"],
                    cwe=best_sig.get("cwe", ""),
                    confidence=best_confidence,
                    source_file=source_file,
                    phase=phase,
                    host=host,
                    url=url,
                    matched_patterns=[s["category"] for s, _, _ in matches],
                    risk_score=0.0,
                )
            )

        # Compute risk scores with host density
        for v in classified:
            density = host_finding_counts.get(v.host, 1) if v.host else 1
            v.risk_score = self._compute_risk_score(v.severity, v.confidence, density)
            self._global_counts[v.category] += 1
            self._phase_counts[v.phase][v.category] += 1

        # Sort by risk_score descending
        classified.sort(key=lambda v: v.risk_score, reverse=True)

        log(
            "ok",
            f"ml_vuln: classified {len(classified)} vulnerabilities across "
            f"{len(host_finding_counts)} hosts",
        )

        return classified

    def get_summary(self, classified: List[ClassifiedVulnerability]) -> Dict[str, Any]:
        """Generate a summary of classified vulnerabilities."""
        by_severity: Counter = Counter()
        by_category: Counter = Counter()
        by_host: Dict[str, Counter] = defaultdict(Counter)

        for v in classified:
            by_severity[v.severity] += 1
            by_category[v.category] += 1
            if v.host:
                by_host[v.host][v.category] += 1

        return {
            "total": len(classified),
            "by_severity": dict(by_severity),
            "by_category": dict(by_category),
            "by_host": {h: dict(c) for h, c in by_host.items()},
            "avg_confidence": round(
                sum(v.confidence for v in classified) / max(1, len(classified)), 3
            ),
            "avg_risk_score": round(
                sum(v.risk_score for v in classified) / max(1, len(classified)), 3
            ),
        }

    def export_classified(self, classified: List[ClassifiedVulnerability], outdir: Path) -> Path:
        """Export classified vulnerabilities to JSON."""
        out = ensure(outdir / "classified_vulns.json")
        data = [v.to_dict() for v in classified]
        out.write_text(json.dumps(data, indent=2, default=str))
        log("OK", f"ml_vuln: exported {len(data)} classified vulns → {out}")
        return out


def classify_findings(
    outdir: Path,
    *,
    min_confidence: float = 0.5,
    host_filter: Optional[str] = None,
) -> List[ClassifiedVulnerability]:
    """Convenience function for one-shot vulnerability classification.

    Args:
        outdir: Output directory containing scan artifacts.
        min_confidence: Minimum confidence threshold.
        host_filter: Only classify findings for this host.

    Returns:
        List of ClassifiedVulnerability sorted by risk_score.
    """
    classifier = VulnerabilityClassifier()
    return classifier.classify(outdir, host_filter=host_filter, min_confidence=min_confidence)
