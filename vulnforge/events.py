"""
vulnforge.events — lightweight in-process event bus for real-time data streaming.
...
class EventBus:
    ...
    def emit(self, event_type: str, data: Optional[Dict[str, Any]] = None) -> Event:
        ...
        """
        event = Event(type=event_type, data={} if data is None else data)
        with self._lock:
            self._event_count += 1
        ...
        for cb in targets + wildcards:
            try:
                ...
                logging.getLogger("vulnforge.events").warning(
                    "subscriber error for %s: %s", event_type, exc
                )
        return event
