...
async def phase_14_ORIGIN(
    ...
    for url in favicon_urls:
        try:
            ...
            if data:
                fav_hash = _mmh3_hash(data) & 0xFFFFFFFF
                findings.append(f"favicon_hash={fav_hash} (url={url})")
                findings.append(
                    f"  Shodan: https://www.shodan.io/search?query=http.favicon.hash:{fav_hash}"
                )
                findings.append(
                    f"  Shodan (org): https://www.shodan.io/search?query=org:%22Cloudflare%22+http.favicon.hash:{fav_hash}"
                )
                break
    ...
    if t.has("dig"):
        rc, mx_stdout, _ = await _run_cmd_clear_proxy(
            ["dig", "+short", _DNS_RESOLVER, "mx", domain],
            timeout=15,
        )
                    ...
                    if t.has("dig"):
                        try:
                            rc2, out2, _ = await _run_cmd_clear_proxy(
                                ["dig", "+short", _DNS_RESOLVER, mx_host.rstrip(".")],
                                timeout=10,
                            )
        ...
        try:
            ...
            for ns_line in ns_stdout.decode(errors="ignore").splitlines() if rc_ns == 0 else []:
                ...
                try:
                    ...
                    _axfr_has_data = "SOA" in axfr_text and "Transfer failed" not in axfr_text
                    if _axfr_has_data:
    ...
    if t.has("dig"):
        for rec, label in (("txt", "SPF"), ("txt", "DMARC"), ("txt", "DKIM")):
            query = (
                f"_dmarc.{domain}"
                if label == "DMARC"
                else (f"default._domainkey.{domain}" if label == "DKIM" else domain)
            )
            try:
...
_CLOUD_PROVIDERS: List[Dict[str, Any]] = [
    {"name": "AWS", "domain": "s3.amazonaws.com", "fmt": "{bucket}.s3.amazonaws.com"},
    {
        ...
        "fmt": "{bucket}.s3-ap-southeast-1.amazonaws.com",
    },
    {
        "name": "AWS-EU",
        "domain": "s3-eu-west-1.amazonaws.com",
        "fmt": "{bucket}.s3-eu-west-1.amazonaws.com",
    },
    {"name": "GCP", "domain": "storage.googleapis.com", "fmt": "{bucket}.storage.googleapis.com"},
    {
        "name": "GCP-ASIA",
        "domain": "storage.googleapis.com",
        "fmt": "{bucket}.storage.googleapis.com",
    },
    {"name": "Azure", "domain": "blob.core.windows.net", "fmt": "{bucket}.blob.core.windows.net"},
    {
        "name": "Azure-Private",
        "domain": "blob.core.windows.net",
        "fmt": "{bucket}.blob.core.windows.net",
    },
    {"name": "DO", "domain": "digitaloceanspaces.com", "fmt": "{bucket}.digitaloceanspaces.com"},
    {"name": "Linode", "domain": "linodeobjects.com", "fmt": "{bucket}.linodeobjects.com"},
    {
        "name": "Backblaze",
        "domain": "backblazeb2.com",
        "fmt": "{bucket}.s3.us-west-004.backblazeb2.com",
    },
    {"name": "Alibaba", "domain": "oss.aliyuncs.com", "fmt": "{bucket}.oss.aliyuncs.com"},
    {
        "name": "Hetzner",
        "domain": "your-objectstorage.com",
        "fmt": "{bucket}.your-objectstorage.com",
    },
]
...
async def phase_50_BUCKET_PERMS(
    ...
    async def _probe_bucket(url: str, label: str) -> List[str]:
        ...
        try:
            req = urllib.request.Request(
                url, method="GET", headers={"User-Agent": "Mozilla/5.0", **_b_extra_headers}
            )
        ...
        try:
            put_req = urllib.request.Request(
                url + "/.vulnforge_permtest",
                data=b"vulnforge-permtest",
                method="PUT",
        ...
        if "amazonaws.com" in host_lower:
            for region in ["us-east-1", "eu-west-1", "us-west-2", ""]:
                ...
                url = (
                    ...
                    if region
                    else f"https://{base}.s3.amazonaws.com/"
                )
