...
install_python_tools() {
  ...
  if ! command -v sqlmap >/dev/null 2>&1; then
    ...
    if [[ -d /opt/sqlmap ]]; then
      ...
      sudo git clone --depth 1 --branch 2.8.13 https://github.com/sqlmapproject/sqlmap.git /opt/sqlmap
  ...
  if ! command -v secretfinder >/dev/null 2>&1; then
    if [[ ! -d /opt/SecretFinder ]]; then
      sudo git clone --depth 1 --branch master https://github.com/m4ll0k/SecretFinder.git /opt/SecretFinder 2>/dev/null || true
    fi
  ...
  # xnLinkFinder — JS endpoint discovery
  if ! command -v xnLinkFinder >/dev/null 2>&1; then
    pip3 install --user xnLinkFinder 2>/dev/null || pip3 install --user --break-system-packages xnLinkFinder 2>/dev/null || warn "xnLinkFinder install failed"
  fi
  ...
  if ! command -v gitdumper >/dev/null 2>&1; then
    if [[ ! -d /opt/gitdumper ]]; then
      sudo git clone --depth 1 --branch 1.4.2 https://github.com/arthaud/git-dumper.git /opt/gitdumper 2>/dev/null || true
    fi
...
GITEOF
  ...
  if ! command -v commix >/dev/null 2>&1; then
    if [[ ! -d /opt/commix ]]; then
      sudo git clone --depth 1 --branch v3.8 https://github.com/commixproject/commix.git /opt/commix 2>/dev/null || true
    fi
  ...
  if ! command -v corsy >/dev/null 2>&1; then
    if [[ ! -d /opt/Corsy ]]; then
      sudo git clone --depth 1 --branch main https://github.com/s0md3v/Corsy.git /opt/Corsy 2>/dev/null || true
    fi
  ...
  if ! command -v smuggler >/dev/null 2>&1; then
    if [[ ! -d /opt/smuggler ]]; then
      sudo git clone --depth 1 --branch main https://github.com/defparam/smuggler.git /opt/smuggler 2>/dev/null || true
    fi
...
install_testssl() {
  ...
  if ! command -v testssl.sh >/dev/null 2>&1 && ! command -v testssl >/dev/null 2>&1; then
    ...
    sudo git clone --depth 1 --branch 3.2 https://github.com/drwetter/testssl.sh.git /opt/testssl.sh
