...
Searches for vulnforge.cfg in:
  1. --config CLI flag (explicit path)
  2. ./vulnforge.cfg (current directory)
  3. ~/.config/vulnforge/vulnforge.cfg (XDG)

Falls back gracefully if no config file exists.

Wizard profiles are stored in ~/.config/vulnforge/profiles/ as TOML files.
"""
...
CONFIG_FILENAME = "vulnforge.cfg"

# Default search paths (in priority order)
_CONFIG_SEARCH_PATHS = [
    Path("vulnforge.cfg"),
    Path.home() / ".config" / "vulnforge" / "vulnforge.cfg",
]
        ...
        for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
            ...
            if "=" in line:
                ...
                if " #" in val:
                    ...
                    continue
                _quoted = val.startswith(('"', "'")) and len(val) >= 2
                val = val.strip('"').strip("'")
                # Parse TOML-style arrays: ["a", "b"] or [1, 2]
                if val.startswith("[") and val.endswith("]"):
                    inner = val[1:-1].strip()
                    if not inner:
                        val = []  # type: ignore[assignment]
                    else:
                        items: list = []
                            ...
                            if item.lower() in ("true", "yes"):
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        items.append(item)
                        val = items  # type: ignore[assignment]
                # Type coercion
                elif _quoted:
                    pass
...
def apply_config_to_args(cfg: Dict[str, Any], args: Any) -> Any:
    ...
    if isinstance(gh_tokens, list) and gh_tokens:
        ...
        if tokens_file.exists():
            existing = set(tokens_file.read_text(encoding="utf-8").strip().splitlines())
...
def _set_arg_if_default(args: Any, attr: str, val: Any) -> None:
    ...
    NOTE: can't distinguish a user-explicit ``--rate-limit 10`` from the default of 10.
    If the user passes a value that matches the parser default, it is treated as
    "not set" and may be overridden by config. This is a known limitation.
    """
    if val is None:
        return
    ...
    if attr in ("proxy", "vuln_proxy") and isinstance(val, str) and val.strip():
        ...
        if not val.startswith(
            ...
            print(
                f"[W] config: invalid proxy URL scheme in {attr}={val!r}; ignoring", file=sys.stderr
            )
            return
    if isinstance(val, bool) and attr in ("proxy", "vuln_proxy"):
        val = str(val).lower()
...
def generate_example_config() -> str:
    ...
    return """# VulnForge configuration file
# Place this as vulnforge.cfg in the project root or ~/.config/vulnforge/
...
_PROFILES_DIR = Path.home() / ".config" / "vulnforge" / "profiles"
