"""VulnForge exception hierarchy.

All exceptions inherit from ``VulnForgeError`` so callers can catch
...
class PipelineError(VulnForgeError):
...
class ToolError(VulnForgeError):
...
class NetworkError(VulnForgeError):
...
class PluginError(VulnForgeError):
...
class IntegrationError(VulnForgeError):
