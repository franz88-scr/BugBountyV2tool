...
class ToolHealthMonitor:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            for name, metrics in data.get("tools", {}).items():
                fields = {k: v for k, v in metrics.items() if k in _valid_fields}
                tm = ToolMetrics(**fields)
                if tm.disabled:
    ...
    def record_failure(self, tool_name: str, reason: str = "") -> None:
        ...
        with self._lock:
            ...
            if tm.consecutive_failures >= self.FAILURE_THRESHOLD:
                tm.disabled = True
                tm.disabled_reason = (
                    f"Auto-disabled after {tm.consecutive_failures} consecutive failures: {reason}"
                )
                self._disabled_tools.add(tool_name)
            log(
                "warn",
                f"Tool health: {tool_name} auto-disabled ({tm.consecutive_failures} failures)",
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        healthy = sum(
            1 for tm in self._tools.values() if tm.success_rate >= 0.5 and not tm.disabled
        )
        degraded = sum(
            1 for tm in self._tools.values() if 0.2 <= tm.success_rate < 0.5 and not tm.disabled
        )
