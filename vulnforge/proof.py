...
def generate_pocs(finding: str, vuln_type: str) -> List[Dict[str, str]]:
    ...
    if vuln_type in ("xss", "stored_xss"):
            ...
            pocs.append(
                {
                    "type": "XSS",
                    "payload": payload,
                    "delivery": f"Inject into parameter: {param}"
                    if param
                    else "Inject into vulnerable field",
            ...
            pocs.append(
                {
                    "type": "SSRF",
                    "payload": payload,
                    "delivery": f"Set parameter {param} to: {payload}"
                    if param
                    else f"Set URL to: {payload}",
                    "url": url or "(URL from finding)",
                }
            )
            ...
            pocs.append(
                {
                    "type": "SSTI",
                    "payload": payload,
                    "delivery": f"Inject into template parameter: {param}"
                    if param
                    else "Inject into template",
                    "url": url or "(URL from finding)",
                }
            )
            ...
            pocs.append(
                {
                    "type": "SQLi",
                    "payload": payload,
                    "delivery": f"Append to parameter {param}"
                    if param
                    else "Append to input field",
                    "url": url or "(URL from finding)",
                }
            )
            ...
            pocs.append(
                {
                    "type": "LFI",
                    "payload": payload,
                    "delivery": f"Replace file path in parameter {param}"
                    if param
                    else "Replace file path",
                    "url": url or "(URL from finding)",
                }
            )
        ...
        for payload_template in _CSRF_PAYLOADS:
            payload = payload_template.replace("{url}", url or "TARGET_URL")
            pocs.append(
                {
                    "type": "CSRF",
                    "payload": payload,
...
def generate_all_pocs(outdir: Path, max_pocs: int = 2000) -> Path:
    """Generate PoCs for all findings and write to outdir/poc/.

    Args:
        max_pocs: Upper bound on the total number of PoCs generated, so a
            scan with tens of thousands of finding lines cannot balloon memory.
    """
    ...
    for art in ARTIFACTS:
        ...
        for line in read_lines(p):
            ...
            if not text:
                continue
            for poc in generate_pocs(text, art.vuln_type):
                if budget <= 0:
                    break
                poc["finding"] = text
                poc["source"] = art.display_name
                file_pocs.append(poc)
                budget -= 1
