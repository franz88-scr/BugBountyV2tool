"""
vulnforge.dashboard_server — live web dashboard with SSE streaming.
...
<title>VulnForge Live Dashboard</title>
<style>
...
<div class="header">
  ...
  <h1>&#9881; VulnForge Live Dashboard</h1>
...
function renderPhases(){
  ...
  for(var[name,s]of sorted){
    ...
    html+='<div class="phase '+escapeHtml(s.status)+'"><span class="icon">'+icon+'</span><span class="name">'+escapeHtml(name)+'</span><span class="time">'+escapeHtml(String(timeStr))+'</span></div>';
  }
  el.innerHTML=html;
}
...
function renderFindings(){
  ...
  for(var f of filtered.slice(-100).reverse()){
    html+='<div class="finding"><span class="sev '+escapeHtml(f.severity)+'"></span><span class="text">'+escapeHtml(f.text.substring(0,120))+'</span><div class="meta">'+escapeHtml(f.source)+' &middot; '+escapeHtml(f.severity.toUpperCase())+'</div></div>';
  }
  el.innerHTML=html;
}
