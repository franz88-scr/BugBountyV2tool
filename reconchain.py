...
async def run_parallel(jobs: List[Tuple[str, List[str], int]],
    ...
    coros = [_run(n, c, t, outdir) for n, c, t in jobs if c]
    ...
    def stop(self) -> Path:
        ...
        if self.proc.poll() is None:
            ...
            try:
                ...
                self.proc.kill()
        # parse the JSON-line event stream
        events: List[dict] = []
...
async def phase_A1(domain: str, outdir: Path, t: Tools,
    ...
    if "assetfinder" in t.have("assetfinder"):
        jobs.append(("assetfinder", ["bash", "-c",
            f"assetfinder --subs-only {shlex_quote(domain)} > {outdir / 'subs_assetfinder.txt'}"],
            600))
    if not jobs:
...
async def phase_C1(outdir: Path, t: Tools, only: set, skip: set,
    ...
    for bin_, args in [
        ("gau",       ["gau", "--threads", "5", "--subs", "--blacklist",
                       "ttf,woff,svg,png,jpg,gif,ico,css",
                       "--o", str(outdir / "urls_gau.txt")]),
        ("waybackurls", ["waybackurls", str("__DOMAIN__")]),
        ("gospider",  ["gospider", "-q", "-s", str(hosts),
                       "-o", str(outdir / "urls_gospider.txt")]),
    ]:
        if t.has(bin_):
            cmd = [a if a != "__DOMAIN__" else
                   "PLACEHOLDER" for a in args]
            if bin_ == "waybackurls":
                # waybackurls is per-domain; feed first line
                cmd = ["bash", "-c",
                       f"waybackurls $(head -1 {shlex_quote(str(hosts))}) "
                       f"> {outdir / 'urls_wayback.txt'}"]
            g1.append((bin_, cmd, 1800))
    ...
    if t.has("subjs"):
        g2.append(("subjs",
            ["bash", "-c",
             f"subjs -i {shlex_quote(str(hosts))} > {outdir / 'urls_subjs.txt'}"],
             1200))
...
async def phase_C2(outdir: Path, t: Tools, only: set, skip: set) -> Dict[str, Any]:
    ...
    # crude filter: any URL containing common JS extensions
    if urls.exists():
        keep = [u for u in read_lines(urls)
                if u.lower().split("?")[0].endswith((".js", ".jsx"))]
        if keep:
        ...
        for u in read_lines(urls)[:3]:
            jobs.append((f"paramspider-{u[:40]}",
                ["bash", "-c",
                 ...
                 f"{outdir / f'params_spider_{abs(hash(u))%9999}.txt'}"], 900))
    if t.has("arjun"):
        jobs.append(("arjun",
        ...
        for u in sample:
            jobs.append((f"ffuf-{u[:32]}",
                ["ffuf", "-silent", "-u", u.rstrip("/") + "/FUZZ",
                 ...
                 "-o", str(outdir / f"ffuf_{abs(hash(u))%9999}.json")], 1500))
    if t.has("kiterunner"):
        for u in sample:
            jobs.append((f"kiterunner-{u[:32]}",
                ["kr", "scan", u, "-w",
                 ...
                 "-o", str(outdir / f"kr_{abs(hash(u))%9999}.json")], 1500))
    if t.has("feroxbuster"):
        for u in sample:
            jobs.append((f"feroxbuster-{u[:32]}",
                ["feroxbuster", "-q", "-u", u, "--no-state",
                 "-o", str(outdir / f"fb_{abs(hash(u))%9999}.txt")], 1800))
    await run_parallel(jobs, outdir)
    n = merge_unique(list(outdir.glob("ffuf_*.json")) + list(outdir.glob("kr_*.json")) +
                     list(outdir.glob("fb_*.txt")), outdir / "fuzz.txt")
...
async def phase_F1(outdir: Path, t: Tools, only: set, skip: set) -> Dict[str, Any]:
    ...
    if t.has("httpx") and t.has("nuclei"):
...
async def phase_F2(outdir: Path, t: Tools, only: set, skip: set) -> Dict[str, Any]:
    ...
    if t.has("testssl.sh") or t.has("testssl"):
        for h in sample:
            jobs.append((f"testssl-{h[:32]}",
                ["bash", "-c",
                 f"testssl.sh --quiet --color 0 {shlex_quote(h)} >> "
                 f"{outdir / 'testssl.txt'}"], 1800))
    if t.has("wpscan"):
        for h in sample:
            if "http" in h:
                jobs.append((f"wpscan-{h[:32]}",
                    ["wpscan", "--url", h, "--no-banner",
                     "--output", str(outdir / f"wpscan_{abs(hash(h))%9999}.txt")],
                    1800))
    await run_parallel(jobs, outdir)
...
async def phase_G(outdir: Path, t: Tools, only: set, skip: set,
    ...
    xss_in.write_text("\n".join(u for u in read_lines(urls)
                                if "=" in u) + "\n")
    if t.has("dalfox"):
        jobs.append(("dalfox",
            ["dalfox", "file", str(xss_in), "--silence",
             "--output", str(outdir / "xss.txt")], 1500))
    if t.has("sqlmap"):
        jobs.append(("sqlmap",
            ["bash", "-c",
             f"sqlmap -m {shlex_quote(str(xss_in))} --batch --level=2 "
             f"--risk=1 --random-agent --output-dir={outdir / 'sqlmap'} "
             f">> {outdir / 'sqlmap.log'}"], 3600))
    ...
    if ssrf_in.exists() and read_lines(ssrf_in):
        ssrf_script = outdir / "ssrf_probe.sh"
        ssrf_script.write_text(
            "#!/usr/bin/env bash\nset -u\n"
            f"OAST={oast_domain or ''}\n"
...
def write_summary(outdir: Path, domain: str, state: dict,
    ...
    payload = {
        ...
        "artifacts": {k: str(v) for k, v in state.get("artifacts", {}).items()},
        "counts": counts,
    }
