...
install_go_tools() {
  ...
  for repo in "${GO_TOOLS[@]}"; do
    i=$((i+1))
    printf "  [%d/%d] %s …\n" "$i" "${#GO_TOOLS[@]}" "$repo"
    # amass uses a multi-package path; handle separately
    if [[ "$repo" == *"amass"* && "$repo" == *"..."* ]]; then
      GO111MODULE=on go install -v "$repo" || warn "amass install had warnings"
    else
...
install_python_tools() {
  ...
  for pkg in secretfinder arjun; do
    if ! python3 -c "import importlib,sys; sys.exit(0 if importlib.util.find_spec('${pkg%finder}') else 1)" 2>/dev/null; then
