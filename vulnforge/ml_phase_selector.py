"""ML-assisted phase selection for VulnForge pipeline.
...
Usage:
    from vulnforge.ml_phase_selector import select_optimal_phases, PhaseSelector
...
from vulnforge.config import DISCOVERY_PHASES, VALID_PHASES
from vulnforge.utils import ensure

# Phases that are always included regardless of budget
MANDATORY_PHASES = {"00-SCOPE"}
...
# Config phase IDs that map to each category (fallback when PHASE_CATEGORIES key not found)
_CONFIG_RECON_PREFIXES = ("00-", "01-", "02-", "03-")
_CONFIG_META_PHASES: frozenset[str] = frozenset()


def _get_phase_category(phase: str) -> str:
    """Determine phase category, handling both PHASE_CATEGORIES keys and config VALID_PHASES IDs."""
    if phase in PHASE_CATEGORIES:
        return PHASE_CATEGORIES[phase]
    if phase in DISCOVERY_PHASES:
        return "discovery"
    if phase in _CONFIG_META_PHASES:
        return "meta"
    if phase.startswith(_CONFIG_RECON_PREFIXES):
        return "recon"
    return "vuln"
...
class PhaseSelector:
    ...
    def record_result(self, phase: str, findings_count: int) -> None:
        ...
        with _FEEDBACK_LOCK:  # type: ignore[attr-defined]
            self._history = self._read_history()
    ...
    def _score_phase(
        ...
        category = _get_phase_category(phase)
        ...
        if category in ("vuln", "discovery"):
            recon_findings = sum(existing_findings.get(p, 0) for p in DISCOVERY_PHASES)
            if recon_findings == 0 and not any(
                existing_findings.get(p, 0) > 0 for p in ["00-SCOPE", "01-RECON"]
            ):
                dep_penalty = 0.3
        ...
        final_score = (
            yield_score * 0.4 + target_boost * 0.3 + dep_penalty * 0.2 + cat_weight * 0.1
    ...
    def suggest_phases(
        ...
        *,
        ...
        selected = [s.phase for s in ranked if s.phase in MANDATORY_PHASES]
        for s in ranked:
            if s.phase not in MANDATORY_PHASES and len(selected) < budget_phases:
                selected.append(s.phase)
        return selected[:budget_phases]
