...
_DEFAULT_PLUGIN_DIR = Path.home() / ".config" / "vulnforge" / "plugins"
...
class PhasePlugin:
    """Base class for VulnForge plugins.
...
class _PluginMeta:
    ...
    def __init__(self, cls: type, source_file: Path, instance: PhasePlugin) -> None:
        self.cls = cls
    ...
    for py_file in sorted(directory.glob("*.py")):
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                f"vulnforge_plugin_{py_file.parent.name}_{py_file.stem}", str(py_file)
            )
        ...
        has_plugin_attrs = any(
            hasattr(mod, attr) for attr in ("register", "PLUGIN_NAME", "phase_name")
        )
        has_phase_subclass = False
        for attr_name in dir(mod):
            obj = getattr(mod, attr_name)
            if inspect.isclass(obj) and issubclass(obj, PhasePlugin) and obj is not PhasePlugin:
                has_phase_subclass = True
                break
        if not has_plugin_attrs and not has_phase_subclass:
            log(
                "warn",
                f"plugin {py_file.name} has no PhasePlugin subclass or plugin attributes, skipping",
            )
            continue
...
def discover_plugins(directories: Optional[List[Path]] = None) -> List[_PluginMeta]:
    ...
    Scans ``~/.config/vulnforge/plugins/`` first, then each directory
...
def register_plugin_to_pipeline(plugins: List[_PluginMeta]) -> None:
    ...
    and VALID_PHASES from vulnforge.phases.__init__.
    """
    import vulnforge.phases as phases_init
