...
_VULN_TYPE_CWE: Dict[str, str] = {
    ...
    "css_injection": "CWE-79",
    "dangling_markup": "CWE-838",
    "service_worker": "CWE-346",
    "wasm_security": "CWE-798",
    "mfa_bypass": "CWE-287",
    "captcha_bypass": "CWE-804",
    "waf_bypass": "CWE-770",
    "redos": "CWE-1333",
    "cookie_toss": "CWE-614",
}
...
class Finding:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> Finding:
        return cls(
            id=d.get("id") or _generate_finding_id(d.get("phase", ""), d.get("evidence", "")),
            phase=d.get("phase", ""),
...
def finding_from_text(
    text: str, phase: str, vuln_type: str = "", tool: str = "", severity: str = ""
) -> Finding:
    ...
    sev = severity if severity else _VULN_TYPE_SEVERITY.get(fv, "info")
...
def _guess_vuln_type(text: str) -> str:
    ...
    if any(kw in lower for kw in ("idor", "insecure direct")):
        return "idor"
    if any(kw in lower for kw in ("open redirect", "url redirect", "redirect to")):
...
class FindingStore:
    ...
    def filter(
        self, severity: str = "", phase: str = "", vuln_type: str = "", host: str = ""
    ) -> List[Finding]:
        results = self.load()
        if severity:
