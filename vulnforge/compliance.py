"""Compliance reporting module for VulnForge.
...
Usage:
    from vulnforge.compliance import generate_compliance_report, Framework
...
class ComplianceControl:
    ...
    description: str
    category: str
    finding_types: List[str]  # vulnforge vuln categories that map to this control
    severity_mapping: Dict[str, str]  # vulnforge severity → compliance impact
...
PCI_DSS_CONTROLS = [
    ComplianceControl(
        ...
        finding_types=["cors", "host_header"],
        severity_mapping={
    ...
    ComplianceControl(
        ...
        finding_types=["default_creds", "exposed_db", "info_leak", "cookie"],
        severity_mapping={
            "critical": "non_compliant",
            "high": "non_compliant",
            "medium": "gap",
    ...
    ComplianceControl(
        ...
        finding_types=["auth_bypass", "default_creds"],
        severity_mapping={
            "critical": "non_compliant",
            "high": "non_compliant",
            "medium": "gap",
...
HIPAA_CONTROLS = [
    ComplianceControl(
        ...
        category="Access Control",
        finding_types=["auth_bypass", "default_creds", "idor", "jwt"],
        severity_mapping={
            "critical": "non_compliant",
            "high": "non_compliant",
            "medium": "gap",
    ...
    ComplianceControl(
        ...
        category="Logging & Monitoring",
        finding_types=["open_redirect", "csrf", "info_leak"],
        severity_mapping={
            "critical": "non_compliant",
            "high": "non_compliant",
            "medium": "gap",
    ...
    ComplianceControl(
        ...
        finding_types=["xss", "stored_xss", "sqli", "file_upload"],
        severity_mapping={
            "critical": "non_compliant",
            "high": "non_compliant",
            "medium": "gap",
    ...
    ComplianceControl(
        ...
        finding_types=["auth_bypass", "session_fixation", "password_spray"],
        severity_mapping={
            "critical": "non_compliant",
            "high": "non_compliant",
            "medium": "gap",
...
SOC2_CONTROLS = [
    ComplianceControl(
        ...
        category="Access Control",
        finding_types=["auth_bypass", "default_creds", "idor", "jwt", "session_fixation"],
        severity_mapping={
            "critical": "non_compliant",
            "high": "partial",
            "medium": "observation",
    ...
    ComplianceControl(
        ...
        category="Vulnerability Management",
        finding_types=["xss", "sqli", "lfi", "rce", "ssti", "xxe", "takeover"],
        severity_mapping={
            "critical": "non_compliant",
            "high": "partial",
            "medium": "observation",
...
def _load_vuln_findings(outdir: Path) -> List[Dict[str, Any]]:
    ...
    if classified_path.exists():
        try:
            return json.loads(classified_path.read_text(encoding="utf-8"))  # type: ignore[no-any-return]
                ...
                findings.append(
                    {
                        ...
                        "text": text,
                        "severity": "high"
                        if art.vuln_type in ("sqli", "xss", "ssrf", "cmdi")
                        else "medium",
                    }
                )
...
def generate_compliance_report(
    ...
    *,
    ...
    log(
        "info",
        f"compliance: assessing {len(controls)} {framework.value} controls against {len(findings)} findings",
    )
...
def generate_all_compliance_reports(
    ...
    *,
    ...
    for framework in Framework:
        results[framework.value] = generate_compliance_report(outdir, framework, domain=domain)
    return results
