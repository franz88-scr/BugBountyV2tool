...
class VulnForgeAPIHandler(BaseHTTPRequestHandler):
    """HTTP request handler for the VulnForge findings API."""
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/v1/health":
            ...
            self._handle_findings_by_severity(params)
    ...
    def _handle_summary(self) -> None:
        ...
        if risk_file.exists():
            try:
                data["risk_score"] = json.loads(
                    risk_file.read_text(encoding="utf-8", errors="ignore")
                )
            except (json.JSONDecodeError, OSError):
                pass
    ...
    def _handle_findings(self, params: Dict[str, list]) -> None:
        if not _outdir or not _outdir.exists():
            _error_response(self, "No scan output directory", 404)
            return
        from vulnforge.finding import FindingStore
        ...
        try:
            limit = max(1, min(int(params.get("limit", ["500"])[0]), 10000))
    ...
    def _handle_findings_by_severity(self, params: Optional[Dict[str, list]] = None) -> None:
        if not _outdir or not _outdir.exists():
            _error_response(self, "No scan output directory", 404)
            return
        from vulnforge.finding import FindingStore
        ...
        try:
            ...
            _error_response(self, "Invalid limit parameter", 400)
            return
    ...
    def _handle_coverage(self) -> None:
        if not _outdir or not _outdir.exists():
            _error_response(self, "No scan output directory", 404)
            return
        from vulnforge.artifacts import get_coverage
    ...
    def _handle_artifacts(self) -> None:
        ...
        from vulnforge.utils import count_nonblank
...
def start_api_server(
    ...
    if host == "0.0.0.0":
        raise ValueError(
            "API server host cannot be 0.0.0.0 (exposes API on all interfaces with no auth). "
            "Use 127.0.0.1 or a specific network IP."
        )
    _outdir = outdir

    _api_server = HTTPServer((host, port), VulnForgeAPIHandler)
