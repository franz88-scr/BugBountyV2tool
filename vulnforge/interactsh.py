...
def _make_callback_handler(instance_id: int) -> type:
    class _Handler(BaseHTTPRequestHandler):
        def do_POST(self) -> None:
            ...
            if select.select([self.rfile], [], [], 5)[0]:
            ...
            if secret:
                ...
                if not hmac.compare_digest(str(body_token), secret) and not hmac.compare_digest(
                    token, secret
                ):
...
class Interactsh:
    ...
    def start_webhook(self, port: int = 0) -> Optional[int]:
        ...
        try:
            ...
            self._httpd = HTTPServer(
                ("127.0.0.1", self._webhook_port), handler_cls, bind_and_activate=False
            )
            self._httpd.socket = sock
            self._httpd.server_address = sock.getsockname()
            self._httpd.server_bind = lambda: None  # type: ignore[method-assign]
    ...
    async def start(self) -> bool:
        ...
        if self.log.exists() and self.log.stat().st_size > 10 * 1024 * 1024:
            self.log.rename(self.log.with_suffix(".log.old"))
        cmd = ["interactsh-client", "-v"]
        cmd_env = None
        if token:
            cmd_env = os.environ.copy()
            cmd_env["INTERACTSH_TOKEN"] = token
        from vulnforge.process import _PIPELINE_CFG, _USE_PROXYCHAINS

        if _PIPELINE_CFG.proxy and not _USE_PROXYCHAINS:
            log(
                "warn",
                "interactsh-client has no native -proxy flag; use SOCKS proxy for proxychains4 routing",
            )
        cmd = _proxify_cmd(cmd)
        try:
            self._log_fh = self.log.open("ab")  # type: ignore[assignment]
            ...
            with _SPAWNED_PIDS_LOCK:
                _SPAWNED_PIDS.append(self.proc.pid)
    ...
    def stop(self) -> Path:
        ...
        try:
            ...
            pass
        with _INSTANCE_LOCK:
            local_cbs = list(self._local_callbacks)
        for cb in local_cbs:
            events.append(
                {
                    "ts": cb.get("_ts"),
        ...
        log(
            "ok",
            f"interactsh: {len(events)} OOB callback(s) captured ({len(self._local_callbacks)} from webhook)",
        )
        self.stop_webhook()
