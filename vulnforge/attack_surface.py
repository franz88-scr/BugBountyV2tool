"""
vulnforge.attack_surface — attack surface graph data generation + interactive HTML visualization.
...
Usage:
    from vulnforge.attack_surface import build_graph, write_attack_surface_html
    ...
    for art in ARTIFACTS:
        ...
        for line in read_lines(fpath):
            ...
            add_node(
                ...
                "vuln",
                finding[:80],
                severity=_guess_severity(finding),
...
def _guess_severity(text: str) -> str:
    t = text.lower()
    if any(w in t for w in ["rce", "critical", "remote code", "sql injection", "sqli"]):
        return "critical"
    if any(w in t for w in ["xss", "ssrf", "lfi", "idor", "high"]):
        return "high"
...
def write_attack_surface_html(outdir: Path, domain: str, graph: Dict[str, Any]) -> Path:
    ...
    graph_json = json.dumps(graph, default=str).replace("</script>", "<\\/script>")
...
<title>Attack Surface — {safe_domain}</title>
<style>
...
<body>
<div id="toolbar">
  <h1>&#9776; {safe_domain} — Attack Surface</h1>
...
function showInfo(n) {{
  ...
  html += '<div class="field"><span class="label">Type:</span> ' + esc(n.type) + '</div>';
  if (n.subtype) html += '<div class="field"><span class="label">Subtype:</span> ' + esc(n.subtype) + '</div>';
  if (n.severity) {{
    const c = SEVERITY_COLORS[n.severity] || '#8b949e';
    html += '<div class="field"><span class="label">Severity:</span> <span style="color:'+c+'">' + esc(n.severity) + '</span></div>';
  }}
  if (n.vuln_type) html += '<div class="field"><span class="label">Vuln Type:</span> ' + esc(n.vuln_type) + '</div>';
  if (n.impact) html += '<div class="field"><span class="label">Impact:</span> ' + esc(n.impact) + '</div>';
  if (n.steps) {{
    html += '<div class="field"><span class="label">Chain Steps:</span></div>';
    n.steps.forEach((s, i) => {{
      html += '<div style="margin-left:12px;color:#c9d1d9">' + (i+1) + '. ' + esc(s.finding || s) + '</div>';
    }});
  }}
  ...
  if (connected.length) {{
    ...
    connected.slice(0, 10).forEach(cn => {{
      html += '<div style="margin-left:12px;color:#8b949e">\u2022 ' + esc(cn.label) + '</div>';
    }});
