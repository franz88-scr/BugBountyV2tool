...
from vulnforge.config import DOS_PHASES, FAST_PHASES, PipelineConfig
...
from vulnforge.resource_monitor import (
    AdaptiveSemaphore,
...
_JOB_SEM: asyncio.Semaphore | AdaptiveSemaphore | None = None
...
def _preflight_memory_check(safe_mode: bool = False) -> None:
    ...
    try:
        ...
        log(
            "info",
            f"Preflight: RAM {total_ram_gb:.1f} GB total, {avail_gb:.1f} GB free | "
        ...
        if swap_total_gb > 0 and swap_free_gb < min_swap:
            log(
                "warn",
                f"Swap is nearly full ({swap_free_gb:.1f} GB free). "
        ...
        if avail_gb < 3.0 and not safe_mode:
            log(
                "warn",
                f"Low available RAM ({avail_gb:.1f} GB). "
...
async def run_pipeline(args: argparse.Namespace) -> int:
    from vulnforge.process import reset_globals
    ...
    if not getattr(args, "no_plugins", False):
        try:
            ...
            from vulnforge.plugin import discover_plugins, register_plugin_to_pipeline
    ...
    if outdir.exists() and not outdir.is_dir():
        from vulnforge.exceptions import OutputPathError
    ...
    try:
        ...
        from vulnforge.audit import log_event as _audit
    ...
    if args.resume and state_path.exists():
        try:
            ...
            if saved.get("domain") and saved.get("domain") != args.domain:
                log(
                    "warn",
                    f"state.json is for domain {saved.get('domain')!r}, not {args.domain!r}; ignoring and starting fresh",
                )
            else:
                ...
                for k, v in (saved.get("artifacts") or {}).items():
                    ...
                    if not resolved.is_relative_to(outdir_resolved):
                        log(
                            "warn",
                            f"state.json artifact path escapes outdir, skipping: {rebased[k]}",
                        )
    ...
    if profile == "quick":
        from vulnforge.config import QUICK_SKIP_PHASES

        skip = skip | QUICK_SKIP_PHASES
        log(
            "info",
            f"Profile 'quick': skipping {len(QUICK_SKIP_PHASES)} redundant/low-signal phases",
        )
    ...
    if _safe_mode:
        ...
        log(
            "info",
            f"Safe mode: phases={jobs}, start={_adaptive_start}, max={_adaptive_max}, max_procs={_adaptive_max_procs}",
        )

    if _adaptive_enabled:
        _adaptive_sem = AdaptiveSemaphore(initial=_adaptive_start)
        _JOB_SEM = _adaptive_sem
        _proc_mod._JOB_SEM = cast(asyncio.Semaphore, _adaptive_sem)
        ...
        log(
            "info",
            f"ResourceMonitor started: initial={_adaptive_start}, max={_resmon._max_limit}, os_procs={MAX_OS_PROCS}, interval={_adaptive_interval}s",
        )
    ...
    if not cookie:
        ...
        cookie = _auto_detect_cookies(
            _outdir_hint, fix_permissions=not getattr(args, "no_fix_permissions", False)
        )
    if cookie:
    ...
    if not vuln_proxy and shutil.which("proxychains4"):
        try:
            ...
            log(
                "info",
                "Tor detected on 127.0.0.1:9050 — auto-enabling --vuln-proxy for vulnerability probing phases",
            )
        except asyncio.CancelledError:
            raise
        except Exception as exc:
    ...
    if _safe_mode:
        log(
            "info",
            "Safe mode: sample sizes halved, delay=0.3s, rate_limit=10, "
    ...
    _tool_health = None
    try:
        from vulnforge.tool_health import get_tool_health_monitor
    ...
    def _apply(name: str, result: Dict[str, Any]) -> None:
        ...
        state["artifacts"].update(
            {k: v for k, v in (result or {}).items() if not isinstance(v, str) or Path(v).exists()}
        )
        ...
        state.setdefault("tool_failures", {}).update(
            {
                k: int(v)
                for k, v in _TOOL_RC_REGISTRY.items()
    ...
    if cookie:
        ...
        _fd, _cookie_path = tempfile.mkstemp(
            prefix=".vulnforge_cookie_", suffix=".txt", dir=str(outdir)
        )
        with os.fdopen(_fd, "w") as _cf:
            _cf.write(cookie)
        os.chmod(_cookie_path, 0o600)
        with _CREDENTIAL_LOCK:
            _CREDENTIAL_STORE["COOKIE"] = cookie
    ...
    elif "COOKIE" in _CREDENTIAL_STORE:
        with _CREDENTIAL_LOCK:
    ...
    with _CREDENTIAL_LOCK:
        if _idor_a:
            _CREDENTIAL_STORE["COOKIE_A"] = _idor_a
        elif "COOKIE_A" in _CREDENTIAL_STORE:
            _CREDENTIAL_STORE.pop("COOKIE_A", None)
        if _idor_b:
            _CREDENTIAL_STORE["COOKIE_B"] = _idor_b
        elif "COOKIE_B" in _CREDENTIAL_STORE:
            _CREDENTIAL_STORE.pop("COOKIE_B", None)
        extra_hdrs = list(getattr(args, "extra_headers", []))
        if extra_hdrs:
            _CREDENTIAL_STORE["EXTRA_HEADERS"] = "\n".join(extra_hdrs)
        elif "EXTRA_HEADERS" in _CREDENTIAL_STORE:
        ...
        async with _PHASE_SEM:
            ...
            if _tool_health is not None:
                ...
                if _tool_health.is_disabled(_tool_name):
                    _debug_ts(
                        f"phase {name} skipped — tool {_tool_name} auto-disabled by health monitor"
                    )
                    progress.next(name)
                    return {}
            ...
            try:
                ...
                try:
                    _pop_task = asyncio.create_task(_pop_phase_proxy(name))
                    await asyncio.shield(_pop_task)
                ...
                bus.emit(
                    "phase.complete",
                    {
                        ...
                        "count": sum(
                            1
                            for v in (_phase_result or {}).values()
                            if isinstance(v, str) and v.endswith(".txt")
                        ),
                    },
                )
    ...
    try:
        ...
        if waf_file.exists():
            ...
            _proc_mod._PIPELINE_CFG.waf_detected = any(
                "detected" in wl.lower() and "no waf" not in wl.lower() for wl in waf_lines
            )
        ...
        for name in phases_to_run:
            ...
            if not deps:
                ready.add(name)
        _debug_ts(
            f"pipeline loop starting: {len(ready)} root phases ready, "
        ...
        while len(completed_phases) < len(phases_to_run):
            for name in list(ready):
                if name not in pending_tasks:
                    pending_tasks[name] = asyncio.create_task(_run_phase(name))
                ...
                with _proc_mod._SPAWNED_PIDS_LOCK:
                    for _pid in list(_proc_mod._SPAWNED_PIDS):
                        ...
                        try:
                            ...
                            pass
                _proc_mod._CANCEL_TIMEOUT_EVENT.set()
                pending_tasks[longest].cancel()
                pending_tasks.pop(longest, None)
                phase_started.pop(longest, None)
                _apply(longest, {})
                progress.next(longest)
                scan_status.add_completed(longest)
            ...
            for name, task in list(pending_tasks.items()):
                if task in done_set:
                    ...
                    _debug_ts(
                        f"phase {name} completed ({len(completed_phases)}/{len(phases_to_run)})"
                    )
                    try:
                        from vulnforge.audit import log_event as _audit_phase
                        ...
                        for _sk, _sv in state.items():
                            if _sk in _SAFE_STATE_KEYS and _sk not in _SENSITIVE_KEYS:
                                if _sk == "artifacts" and isinstance(_sv, dict):
                                    _filtered_state[_sk] = {}
                                    for ak, av in _sv.items():
                                        if any(s in ak.lower() for s in _SENSITIVE_KEYWORDS):
                                            continue
                                        if isinstance(av, str):
                                            _p = Path(av)
                                            try:
                                                _rel = _p.relative_to(outdir)
                                                _filtered_state[_sk][ak] = str(_rel)
                                            except ValueError:
                                                _filtered_state[_sk][ak] = _p.name
                                        else:
                                            _filtered_state[_sk][ak] = av
                                else:
        ...
        if _resmon is not None:
            try:
                await asyncio.to_thread(_resmon.stop)
            except asyncio.CancelledError:
                raise
            except Exception:
                pass
        ...
        with _CREDENTIAL_LOCK:
            for _env_k in ("COOKIE_A", "COOKIE_B"):
                _CREDENTIAL_STORE.pop(_env_k, None)
        _cleanup_child_procs()
        ...
        try:
            from vulnforge.target_profile import build_target_profile, save_profile

            _target_profile = build_target_profile(outdir, args.domain)
            save_profile(_target_profile, outdir)
            log(
                "ok",
                f"target profile: {_target_profile.cloud_provider or 'unknown'} cloud, "
                f"{len(_target_profile.tech_stack)} techs",
            )
        except asyncio.CancelledError:
            raise
        ...
        # v3.0: Risk scoring
        try:
            from vulnforge.severity import calculate_risk_score, write_risk_score
        ...
        try:
            from vulnforge.certainty import score_all_findings, write_confidence_report
            ...
            if _conf_scores:
                ...
                log(
                    "ok",
                    f"confidence: {_confirmed} confirmed, {_likely} likely, {len(_conf_scores)} total",
                )
        except asyncio.CancelledError:
            raise
        ...
        try:
            from vulnforge.proof import generate_all_pocs
        ...
        if _attack_graph or counts.get("subdomains", 0) > 0:
            try:
                ...
                if _graph["nodes"]:
                    write_attack_surface_json(outdir, _graph)
        ...
        if _ai_enabled:
            try:
                from vulnforge.ai import configure as ai_configure
        ...
        if _dashboard_port:
            try:
                from vulnforge.dashboard_server import start_dashboard
        ...
        if _bot_platform:
            try:
                from vulnforge.bot import start_bot_thread
