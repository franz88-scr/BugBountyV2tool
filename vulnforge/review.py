...
from vulnforge.utils import C, ensure, log, read_lines
...
class FindingReview:
    ...
    def export_report(self) -> Path:
        ...
        report = {
            "stats": stats,
            "confirmed_findings": [
                {"finding": f, "notes": self.reviews[f].get("notes", "")} for f in confirmed
            ],
            "false_positives": [
                {"finding": f, "notes": self.reviews[f].get("notes", "")} for f in fps
            ],
        }
...
def run_interactive_review(outdir: Path) -> None:
    ...
    for item in unreviewed:
        sev_color = {"critical": "err", "high": "warn", "medium": "y", "low": "ok"}.get(
            item["severity"], "d"
        )
        print(f"\n  {C.get(sev_color, '')}{item['severity'].upper()}{C['r']} {item['source']}")
    ...
    log(
        "ok",
        f"Reviewed {reviewed} findings: {stats['confirmed']} confirmed, {stats['false_positive']} FPs",
    )
