...
def _run_blocking(
    ...
    for _p in (log_path, err_path):
        if _p.is_symlink():
            ...
            try:
                ...
                log("warn", f"symlink escape detected in log path: {_p} → {_real}; removing")
                _p.unlink()
    ...
    try:
        ...
        try:
            ...
            log("warn", f"cannot open log path {log_path}; logging to /dev/null")
        ...
        try:
            ...
            log("warn", f"cannot open stderr log path {err_path}; logging to /dev/null")
...
async def _pop_phase_proxy(name: str = "") -> None:
    ...
    async with lock:
        if not _PROXY_STATE_STACK:
            log("warn", f"_pop_phase_proxy({name}): stack empty, nothing to pop")
            return
...
def _atomic_write_json(path: Path, payload: dict) -> None:
    ...
    if path.is_symlink():
        ...
        if not real.is_relative_to(path.parent.resolve()):
            log("warn", f"atomic_write_json: removing symlink escape: {path} → {real}")
            path.unlink()
    ...
    try:
        ...
        try:
            ...
            log("warn", f"atomic_write_json: refusing to replace symlink target: {path}")
            raise
