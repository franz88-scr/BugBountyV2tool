...
from vulnforge.config import _SAFE_HOST, VALID_PHASES
...
__all__ = [
    ...
    "write_findings",
    "_load_live_hosts",
    # vulnforge.utils (underscored, needed by phases)
    "_is_valid_hostname",
    "_is_under_domain",
    "_existing_artifacts",
    "_get_urlopener",
...
async def _run_cmd_clear_proxy(cmd: List[str], timeout: int = 10) -> Tuple[int, bytes, bytes]:
    ...
    if sem is not None:
        async with sem:
            return await _do_run()  # type: ignore[no-any-return]
    return await _do_run()  # type: ignore[no-any-return]
...
def _request(host: str, path: str, timeout: int = 10) -> bytes:
    ...
    if not url.startswith("http"):
        url = "https://" + url
    try:
        resp = opener.open(url, timeout=timeout)  # type: ignore[attr-defined]
