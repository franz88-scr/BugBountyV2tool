...
async def phase_137_EMAILFINDER(
    ...
    try:
        ...
        req = urllib.request.Request(crt_url, headers={"User-Agent": "vulnforge/2.0"})
...
# ─── 138-METAGOOFIL ───────────────────────────────────────────────
async def phase_138_METAGOOFIL(
    domain: str, outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, force: bool = False
) -> Dict[str, Any]:
    if "138-METAGOOFIL" in skip:
        return {}
    await _throttle_rate()
    ...
    try:
        ...
        rc, stdout, _ = await _run_limited(
            cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.DEVNULL, timeout=120
        )
        if stdout:
...
# ─── 139-PORCHPIRATE ──────────────────────────────────────────────
async def phase_139_PORCHPIRATE(
    domain: str, outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, force: bool = False
) -> Dict[str, Any]:
    if "139-PORCHPIRATE" in skip:
        return {}
    await _throttle_rate()
        ...
        for path in paths_to_check:
            try:
                ...
                req = urllib.request.Request(
                    url, headers={"User-Agent": "vulnforge/2.0"}, method="HEAD"
                )
...
# ─── 140-DORKHUNTER ───────────────────────────────────────────────
async def phase_140_DORKHUNTER(
    domain: str, outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, force: bool = False
) -> Dict[str, Any]:
    if "140-DORKHUNTER" in skip:
        return {}
    await _throttle_rate()
...
async def phase_142_GITHUBSUB(
    ...
    if "142-GITHUBSUB" in skip:
        return {}
    await _throttle_rate()
    out = ensure(Path(outdir) / "142-GITHUBSUB.txt")
    subs = set()
    github_token = os.environ.get("GITHUB_TOKEN", "")
    headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "vulnforge/2.0"}
    if github_token:
...
# ─── 143-TLSX ─────────────────────────────────────────────────────
async def phase_143_TLSX(
    outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, prev: Any = None, force: bool = False
) -> Dict[str, Any]:
    if "143-TLSX" in skip:
        return {}
    await _throttle_rate()
    ...
    if target_file.exists():
        targets = [
            line.strip()
            for line in target_file.read_text(encoding="utf-8", errors="ignore").splitlines()
            if line.strip()
        ][:200]
...
# ─── 144-ANALYTICSRELS ────────────────────────────────────────────
async def phase_144_ANALYTICSRELS(
    domain: str,
    outdir: Path,
    t: Tools,
    only: PhaseSet,
...
# ─── 145-FAVIRECON ────────────────────────────────────────────────
async def phase_145_FAVIRECON(
    outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, prev: Any = None, force: bool = False
) -> Dict[str, Any]:
    if "145-FAVIRECON" in skip:
        return {}
    await _throttle_rate()
    ...
    for url in urls:
        ...
        for path in ["/favicon.ico", "/favicon.png", "/apple-touch-icon.png"]:
            try:
                fav_url = base + path
                req = urllib.request.Request(
                    fav_url, headers={"User-Agent": "vulnforge/2.0"}, method="HEAD"
                )
...
# ─── 146-JSLUICE ──────────────────────────────────────────────────
async def phase_146_JSLUICE(
    outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, prev: Any = None, force: bool = False
) -> Dict[str, Any]:
    if "146-JSLUICE" in skip:
        return {}
    await _throttle_rate()
    ...
    if js_file.exists():
        js_urls = [
            line.strip()
            for line in js_file.read_text(encoding="utf-8", errors="ignore").splitlines()
            if line.strip() and line.strip().endswith(".js")
        ][:50]
...
# ─── 147-SHORTSCAN ────────────────────────────────────────────────
async def phase_147_SHORTSCAN(
    outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, prev: Any = None, force: bool = False
) -> Dict[str, Any]:
    if "147-SHORTSCAN" in skip:
        return {}
    await _throttle_rate()
...
# ─── 148-GRPCURL ──────────────────────────────────────────────────
async def phase_148_GRPCURL(
    outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, prev: Any = None, force: bool = False
) -> Dict[str, Any]:
    if "148-GRPCURL" in skip:
        return {}
    await _throttle_rate()
