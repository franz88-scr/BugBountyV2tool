...
_JS_SECRET_PATTERNS: List[Tuple[str, str]] = [
    ...
    (
        "s3-bucket",
        r"(?:bucket|asset|media|uploads|backup|files|cdn|static)\.(?:s3\.amazonaws\.com|s3-[a-z0-9-]+\.amazonaws\.com)",
    ),
...
_SOURCE_MAP_RE = re.compile(
    r'(?://#\s*sourceMappingURL=|sourceMappingURL=)([^\s"\']+)', re.IGNORECASE
)


async def phase_06_JSINTEL(
    outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, domain: str = "", force: bool = False
) -> Dict[str, Any]:
    if skip & {"06-JSINTEL"}:
        return {}
    ...
    if _USE_PROXYCHAINS:
        ...
        if len(js_lines) > 100:
            ...
            log(
                "info",
                f"06-JSINTEL: downsampled {len(js_lines)} JS URLs to {len(sampled)} for slow network",
            )
    ...
    if t.has("nuclei"):
        ...
        if len(_nuc_js_lines) > _nuc_cap:
            ...
            log(
                "info",
                f"06-JSINTEL: capped nuclei-exposures input to {_nuc_cap} URLs (from {len(_nuc_js_lines)})",
            )
        jobs.append(
            (
                "nuclei-exposures",
    ...
    if secrets_file.exists() and read_lines(secrets_file):
        ...
        if len(filtered) < len(read_lines(secrets_file)):
            log(
                "info",
                f"06-JSINTEL: filtered {len(read_lines(secrets_file)) - len(filtered)} SecretFinder false positives",
            )
