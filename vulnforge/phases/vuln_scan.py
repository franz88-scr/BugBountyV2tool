        ...
        await run_parallel(
            [
                (
                    "tech-scanner",
                    nuclei_base
                    + ["-t", "http/technologies", "-o", str(outdir / "tech.txt")]
                    + _proxy_opt,
                    7200,
                )
            ],
                ...
                [
                    (
                        ...
                        + [
                            ...
                            "-o",
                            str(outdir / "nuclei_headless.txt"),
                        ]
                        + _proxy_opt,
                        3600,
                    )
...
async def phase_10_TLSCMS(
            ...
            if os.path.isfile(_openssl_bin):
                try:
                    ...
                    if _test.returncode < 0:
                        log(
                            "warn",
                            f"10-TLSCMS: {_openssl_bin} segfaults (signal {-_test.returncode}); skipping testssl — using Python TLS fallback only",
                        )
                        testssl_bin = None
    ...
    if testssl_bin:
        ...
        for h in sample:
            try:
                ...
                _cf_urlopen = _get_urlopener()
                _resp = _cf_urlopen(_req, timeout=5)  # type: ignore[operator]
                try:
    ...
    if t.has("wpscan"):
        for h in sample:
            ...
            if not wp_found:
                ...
                try:
                    req = urllib.request.Request(
                        h, method="GET", headers={"User-Agent": "Mozilla/5.0"}
                    )
            ...
            with _CREDENTIAL_LOCK:
                _wps_cookie = _CREDENTIAL_STORE.get("COOKIE", "")
            if _wps_cookie:
                wpscan_cmd.extend(["--cookie", _wps_cookie])
            with _CREDENTIAL_LOCK:
                _wps_headers = _CREDENTIAL_STORE.get("EXTRA_HEADERS", "")
            if _wps_headers:
    ...
    n = merge_unique(
        list(outdir.glob("testssl_*.txt"))
        + list(outdir.glob("testssl_py_*.txt"))
...
async def phase_13_OOB(
    outdir: Path, t: Tools, only: PhaseSet, skip: PhaseSet, oast: Interactsh, force: bool = False
) -> Dict[str, Any]:
    if skip & {"13-OOB"}:
        return {}
...
async def phase_68_DEPCVE(
    ...
    if t.has("trivy") and (outdir / "urls_all.txt").exists():
        ...
        trivy_base = ["trivy", "fs", "--quiet", "--format", "json"]
        # Allow alternative DB mirror via environment variable
        _trivy_db_url = os.environ.get("TRIVY_DB_URL", "")
        if _trivy_db_url:
            trivy_base.extend(["--db-repository", _trivy_db_url])
        await _run(
            "trivy-check",
            trivy_base + ["--output", str(trivy_out), str(tmp_dir)],
            300,
            outdir,
        )
