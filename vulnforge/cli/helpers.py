...
def main() -> int:
    """Main entry point for vulnforge CLI.
    ...
    if getattr(args, "gen_config", False):
        from vulnforge.conf import generate_example_config
    ...
    if getattr(args, "list_plugins", False):
        ...
        from vulnforge.plugin import discover_plugins, list_plugins_cli
    ...
    if getattr(args, "compare", None):
        ...
        from vulnforge.diff import compare_scans
        ...
        total_changes = result.get("total_new", 0) + result.get("total_resolved", 0)
        print(
            f"Comparison complete: {total_changes} changes ({result.get('total_new', 0)} new, {result.get('total_resolved', 0)} resolved)"
        )
        return 0
    ...
    if getattr(args, "batch", ""):
        ...
        for i, domain in enumerate(domains, 1):
            ...
            if not Path(a.out).resolve().is_relative_to(batch_outdir.resolve()) and not Path(
                a.out
            ).resolve().is_relative_to(Path("./out").resolve()):
                log(
                    "warn",
                    f"batch: output path escapes expected directory for '{domain}', skipping",
                )
                continue
            try:
    ...
    if args.status:
        if args.status.lower() == "list":
            ...
            for s in active:
                print(
                    f"  {s.get('domain')} — phase={s.get('phase')} completed={len(s.get('completed_phases', []))}/{s.get('total_phases')} errors={len(s.get('errors', []))}"
                )
            return 0
        ...
        print(
            f"Progress: {len(data.get('completed_phases', []))}/{data.get('total_phases', '?')} phases completed"
        )
    ...
    if hasattr(args, "proxy") and args.proxy:
        if not args.proxy.startswith(
            ("http://", "https://", "socks4://", "socks5://", "socks5h://", "socks4a://")
        ):
            parser.error(
                f"invalid proxy URL scheme: {args.proxy!r} (must start with http://, https://, socks4://, socks5://, socks5h://, or socks4a://)"
            )

    if hasattr(args, "vuln_proxy") and args.vuln_proxy:
        if not args.vuln_proxy.startswith(
            ("http://", "https://", "socks4://", "socks5://", "socks5h://", "socks4a://")
        ):
    ...
    if args.quiet:
        from vulnforge.utils import log as _quiet_log

        def _quiet_log_impl(lvl, msg):
            if lvl in ("ok", "err", "warn"):
                _quiet_log(lvl, msg)

        import vulnforge.utils as _utils

        _utils.log = _quiet_log_impl
        import vulnforge.phases as _phases

        _phases.log = _quiet_log_impl  # type: ignore[attr-defined]
        import vulnforge.reporting as _rep

        _rep.log = _quiet_log_impl
        import vulnforge.pipeline as _pl
    ...
    try:
        if args.daemon:
            ...
            for domain in domains:
                fd, pidfile_path = tempfile.mkstemp(
                    prefix=f"vulnforge_{domain.replace('.', '_')}_", suffix=".pid"
                )
                try:
                    os.write(fd, b"")
                    os.close(fd)
                    proc = subprocess.Popen(
                        [sys.executable] + daemon_args + ["-d", domain],
                ...
                log(
                    "info",
                    f"daemon started for {domain} (PID {proc.pid}); check status with: --status {domain}",
                )
            return 0
