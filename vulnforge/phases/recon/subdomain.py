...
async def phase_01_RECON(
    ...
    # CT Log sources: crt.sh and certspotter for Certificate Transparency enumeration
    _ct_out = outdir / "subs_ctlogs.txt"
    if not (resume and _ct_out.exists() and count_nonblank(_ct_out) > 0):

        async def _fetch_ct_logs() -> Set[str]:
            ct_subs: Set[str] = set()

            def _fetch_crtsh() -> Set[str]:
                subs: Set[str] = set()
                try:
                    ct_url = f"https://crt.sh/?q=%25.{domain}&output=json"
                    ct_req = urllib.request.Request(ct_url, headers={"User-Agent": "Mozilla/5.0"})
                    _ct_urlopen = _get_urlopener()
                    ct_resp = _ct_urlopen(ct_req, timeout=10)  # type: ignore[operator]
                    ct_data = json.loads(ct_resp.read().decode("utf-8", errors="ignore"))
            ...
            def _fetch_certspotter() -> Set[str]:
                subs: Set[str] = set()
                try:
                    cs_url = f"https://api.certspotter.com/v1/issuances?domain={domain}&include_subdomains=true&expand=dns_names"
                    cs_req = urllib.request.Request(cs_url, headers={"User-Agent": "Mozilla/5.0"})
                    _cs_urlopen = _get_urlopener()
                    cs_resp = _cs_urlopen(cs_req, timeout=10)  # type: ignore[operator]
                    cs_data = json.loads(cs_resp.read().decode("utf-8", errors="ignore"))
            ...
            crt_results, cs_results = await asyncio.gather(
                asyncio.to_thread(_fetch_crtsh),
                asyncio.to_thread(_fetch_certspotter),
            )
    ...
    if not jobs:
        if any(p.exists() for p in _a1_sources):
            n = merge_unique(
                _a1_sources,
                out,
                validator=lambda s: _is_valid_hostname(s) and _is_under_domain(s, domain),
            )
            if n == 0:
                out.touch()
