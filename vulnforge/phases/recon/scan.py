...
async def phase_04_SCAN(
    ...
    if ports_file.exists() and read_lines(ports_file) and t.has("nmap"):
        ...
        if _skipped:
            log(
                "info",
                f"04-SCAN: nmap-sv allowlist: {len(host_ports)} live hosts, skipped {_skipped} wildcard hosts",
            )
    ...
    if raw_hosts.exists() and read_lines(raw_hosts):
        ...
        if _active_lines:
            ...
            log(
                "info",
                f"04-SCAN: {len(_active_lines)}/{len(read_lines(raw_hosts))} non-404 hosts for crawling",
            )
    ...
    for cand in candidates[: _PIPELINE_CFG.sample_urls_fuzz]:
        ...
        try:
            ...
            req = urllib.request.Request(
                url, headers={"User-Agent": "Mozilla/5.0", **_tv_extra_headers}
            )
