...
from vulnforge.utils import ensure, log

DEFAULT_REGISTRY_URL = "https://registry.vulnforge.dev/api/v1"
FALLBACK_REGISTRY_URL = "https://raw.githubusercontent.com/vulnforge/plugins/main/registry.json"

# Local plugin directories searched for installed plugins
_PLUGIN_SEARCH_PATHS = [
    Path.home() / ".vulnforge" / "plugins",
    Path("/usr/local/share/vulnforge/plugins"),
]
...
class PluginManifest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "tags": list(self.tags),
            "phase": self.phase,
            "min_vulnforge": self.min_vulnforge,
            "max_vulnforge": self.max_vulnforge,
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "PluginManifest":
        return cls(
            ...
            tags=d.get("tags", []),
            phase=d.get("phase", ""),
            min_vulnforge=d.get("min_vulnforge", ""),
            max_vulnforge=d.get("max_vulnforge", ""),
...
class PluginMarketplace:
    ...
    def __init__(
        ...
        self._plugin_dir = plugin_dir or Path.home() / ".vulnforge" / "plugins"
    ...
    def _fetch_registry(self, endpoint: str = "") -> Any:
        ...
        try:
            ...
            try:
                ...
                log("warn", "marketplace: fallback registry also unavailable")
                return {}
    ...
    def search(self, query: str = "", *, tags: Optional[List[str]] = None) -> List[PluginManifest]:
        ...
        for p_dict in plugins_raw:
            ...
            if query:
                q = query.lower()
                searchable = (
                    f"{manifest.name} {manifest.description} {' '.join(manifest.tags)}".lower()
                )
    ...
    def install(self, name: str, *, force: bool = False) -> Optional[PluginManifest]:
        ...
        try:
            ...
            tmp_dir = tempfile.mkdtemp(prefix="vulnforge_marketplace_")

            # Download
            req = urllib.request.Request(download_url)
            with urllib.request.urlopen(req, timeout=60) as resp:
                content_length = int(resp.headers.get("Content-Length", 0))
                _MAX_PLUGIN_SIZE = 10 * 1024 * 1024  # 10MB
                if content_length > _MAX_PLUGIN_SIZE:
                    log(
                        "err",
                        f"marketplace: plugin '{name}' too large ({content_length} bytes, max {_MAX_PLUGIN_SIZE})",
                    )
                ...
                with open(archive_path, "wb") as f:
                    ...
                    while True:
                        ...
                        if not chunk:
                            break
                        total += len(chunk)
                        if total > _MAX_PLUGIN_SIZE:
                            log("err", f"marketplace: plugin '{name}' download exceeded size limit")
                            f.close()
                            shutil.rmtree(tmp_dir, ignore_errors=True)
                            return None
                        f.write(chunk)
                ...
                if sys.version_info >= (3, 12):
                    ...
                    for member in tar.getmembers():
                        ...
                        if member.issym() or member.islnk():
                            log(
                                "err",
                                f"marketplace: refusing to extract {member.name} (symlink)",
                            )
                            shutil.rmtree(tmp_dir, ignore_errors=True)
                            return None
                        if not abs_member.startswith(abs_tmp + os.sep) and abs_member != abs_tmp:
                            log(
                                "err",
                                f"marketplace: refusing to extract {member.name} (path traversal)",
                            )
                            shutil.rmtree(tmp_dir, ignore_errors=True)
                            return None
            ...
            extracted_dirs = [
                d for d in Path(tmp_dir).iterdir() if d.is_dir() and not d.name.startswith(".")
            ]
    ...
    def get_local_plugins(self) -> List[Dict[str, Any]]:
        ...
        """
        results = []
        search_dirs = [self._plugin_dir] + [p for p in _PLUGIN_SEARCH_PATHS if p.exists()]
        seen = set()
