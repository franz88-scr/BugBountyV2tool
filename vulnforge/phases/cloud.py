            ...
            try:
                ...
                if status == 200:
                    ...
                    if any(
                        ...
                        for kw in ("password", "secret", "token", "api_key", "aws_secret")
                    ):
                        findings.append(
                            f"[cicd-secrets] {host} path={path} detail=Potential secrets in CI/CD file"
                        )
            except Exception:
                pass
        ...
        try:
            ...
            if status in (200, 401):
                ...
                try:
                    ...
                    cat_status, cat_headers, cat_body = await _async_urlopen(
                        _urlopen, cat_req, timeout=10
                    )
                    if cat_status == 200:
...
async def phase_129_K8S(
    ...
    for host in hosts:
        ...
        try:
            ...
            pass
        dashboard_url = (
            f"https://{host}/api/v1/namespaces/kube-system/services/kubernetes-dashboard"
        )
        try:
            ...
            try:
                ...
                if status == 200:
                    ...
                    for m in aws_key_re.finditer(body_str):
                        findings.append(
                            f"[terraform-secrets] {host} type=AWS_ACCESS_KEY detail={m.group()}"
                        )
                    for m in pwd_re.finditer(body_str):
                        findings.append(
                            f"[terraform-secrets] {host} type=PASSWORD detail={m.group(2)[:50]}"
                        )
                    for m in ip_re.finditer(body_str):
                        findings.append(
                            f"[terraform-secrets] {host} type=PRIVATE_IP detail={m.group()}"
                        )
                    for m in token_re.finditer(body_str):
                        findings.append(
                            f"[terraform-secrets] {host} type=API_TOKEN detail={m.group(2)[:50]}"
                        )
            except Exception:
                pass
        await _throttle_rate()
...
async def phase_133_APIVERSION(
    ...
    for url in api_urls:
        ...
        for tag, variant_path in variants:
            ...
            try:
                req = urllib.request.Request(
                    variant_url, headers={"Accept": "*/*", **_extra_h}, method="GET"
                )
                status, resp_headers, body_bytes = await _async_urlopen(_urlopen, req, timeout=12)
                if status == 200:
                    findings.append(
                        f"[api-version-bypass] {url} {tag}={variant_path} status={status}"
                    )
        ...
        if internal_path != path:
            ...
            try:
                req = urllib.request.Request(
                    internal_url, headers={"Accept": "*/*", **_extra_h}, method="GET"
                )
                status, resp_headers, body_bytes = await _async_urlopen(_urlopen, req, timeout=12)
                if status == 200:
    ...
    for host in hosts:
        ...
        for hh, status, content_len, title in results[1:]:
            ...
            if status_diff or len_diff > 100 or title_diff:
                findings.append(
                    f"[vhost-found] {ip} host={hh} status={status} len={content_len} title={title}"
                )
...
async def phase_136_RATELIMITBYPASS(
    ...
    for url in urls:
        ...
        for hdr_name, ip_list in ip_rotation_headers:
            ...
            try:
                ...
                if status == 200 and baseline_status in (429, 403):
                    findings.append(
                        f"[ratelimit-bypass] {url} method={hdr_name}:{fake_ip} status={status}"
                    )
                    break
            except urllib.error.HTTPError as e:
                if e.code == 200 and baseline_status in (429, 403):
                    findings.append(
                        f"[ratelimit-bypass] {url} method={hdr_name}:{fake_ip} status={e.code}"
                    )
                    break
            except (urllib.error.URLError, OSError, socket.timeout):
                pass
        ...
        for method in ["GET", "POST", "PUT", "PATCH", "OPTIONS"]:
            ...
            try:
                req = urllib.request.Request(
                    url, headers={"Accept": "*/*", **_extra_h}, method=method
                )
                status, resp_headers, body_bytes = await _async_urlopen(_urlopen, req, timeout=12)
                if status == 200 and baseline_status in (429, 403):
                    findings.append(
                        f"[ratelimit-bypass] {url} method=HTTP_{method} status={status}"
                    )
                    break
            except urllib.error.HTTPError as e:
                if e.code == 200 and baseline_status in (429, 403):
                    findings.append(
                        f"[ratelimit-bypass] {url} method=HTTP_{method} status={e.code}"
                    )
                    break
            except (urllib.error.URLError, OSError, socket.timeout):
                pass
        ...
        for transform in case_transforms:
            ...
            try:
                ...
                if status == 200 and baseline_status in (429, 403):
                    findings.append(
                        f"[ratelimit-bypass] {url} method=CASE_VARIATION status={status}"
                    )
                    break
            except urllib.error.HTTPError as e:
                if e.code == 200 and baseline_status in (429, 403):
                    findings.append(
                        f"[ratelimit-bypass] {url} method=CASE_VARIATION status={e.code}"
                    )
                    break
            except (urllib.error.URLError, OSError, socket.timeout):
                pass
        ...
        # Technique 6: Sequential X-Forwarded-For IP rotation (10.0.0.1, 10.0.0.2, ...)
        await _throttle_rate()
        seq_ips = [f"10.0.0.{i}" for i in range(1, 21)]
        for seq_ip in seq_ips[:5]:
            await _throttle_rate()
            try:
                headers = {"Accept": "*/*", "X-Forwarded-For": seq_ip, **_extra_h}
                req = urllib.request.Request(url, headers=headers, method="GET")
        ...
        # Technique 7: Sequential X-Real-IP rotation (10.0.0.1, 10.0.0.2, ...)
        await _throttle_rate()
        seq_real_ips = [f"10.0.0.{i}" for i in range(1, 21)]
        for seq_ip in seq_real_ips[:5]:
            await _throttle_rate()
            try:
                headers = {"Accept": "*/*", "X-Real-IP": seq_ip, **_extra_h}
                req = urllib.request.Request(url, headers=headers, method="GET")
                s, _, _ = await _async_urlopen(_urlopen, req, timeout=12)
                if s == 200 and baseline_status in (429, 403):
                    findings.append(
