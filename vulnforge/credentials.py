"""Credential encryption, rotation, and secure storage for VulnForge.
...
def _derive_key(salt: bytes) -> bytes:
    """Derive a Fernet key from machine-specific entropy + salt via PBKDF2.

    The key is derived using PBKDF2-HMAC-SHA256 with a per-user salt,
    making it resistant to brute-force even when hostname+user are known.
    """
    ...
    return base64.urlsafe_b64encode(hashlib.pbkdf2_hmac("sha256", raw, salt, 600_000)[:32])
...
class CredentialStore:
    ...
    def _encrypt(self, plaintext: str) -> str:
        if not self._supported or self._fernet is None:
            raise RuntimeError(
                "credential store: cryptography package not installed. "
                "Install with: pip install vulnforge[crypto] or pip install cryptography"
            )
