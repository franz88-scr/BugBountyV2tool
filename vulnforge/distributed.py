    ...
    def next_host(self) -> str:
        ...
        with self._lock:
            ...
            if self._strategy == "weighted":
                total = sum(
                    self._weights.get(h, 1.0) for h in self._hosts if self._failures.get(h, 0) < 3
                )
                if total == 0:
...
class HostHealth:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "vulnforge_version": self.vulnforge_version,
...
class SSHScanner:
    ...
    async def _ssh_exec(self, host: str, command: str, timeout: int = 300) -> Dict[str, Any]:
        """Execute a command on a remote host via SSH."""
        import re

        if re.search(r"[;|`\s/]", host):
            raise ValueError(f"Invalid host: {host} contains unsafe characters")
    ...
    def get_healthy_hosts(self, *, min_disk_gb: float = 1.0) -> List[str]:
        ...
        return [
            h
            for h, health in self._health.items()
            if health.reachable and health.disk_free_gb >= min_disk_gb
        ]

    async def distribute_tasks(self, tasks: List[str], outdir: Path) -> List[Dict[str, Any]]:
