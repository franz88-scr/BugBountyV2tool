...
def test_hostname_validation_rejects_bad_labels() -> None:
    assert vulnforge._is_valid_hostname("example.com")
    assert vulnforge._is_valid_hostname("a-b.example.com")
    assert vulnforge._is_valid_hostname("bad_domain.example.com")  # underscores now allowed
    assert not vulnforge._is_valid_hostname("-bad.example.com")
    assert not vulnforge._is_valid_hostname("bad-.example.com")
    assert not vulnforge._is_valid_hostname("localhost")
    assert not vulnforge._is_valid_hostname("example.com;touch /tmp/pwn")


def test_phase_csv_validation() -> None:
    assert vulnforge._parse_phase_csv("01-recon, 10-tlscms,11-inject") == {"01-RECON", "10-TLSCMS", "11-INJECT"}
    with pytest.raises(argparse.ArgumentTypeError):
        vulnforge._parse_phase_csv("01-RECON,NOPE")


def test_parser_help_returns_0() -> None:
    parser = vulnforge.build_parser()
...
def test_domain_validation_in_main() -> None:
    ...
    assert not vulnforge._is_valid_hostname("bad_domain")
...
def test_merge_unique_filters_and_avoids_self_merge(tmp_path: Path) -> None:
    ...
    count = vulnforge.merge_unique([src, dst], dst, vulnforge._is_valid_hostname)

    # count includes pre-existing dst content (old.example.com) plus valid src lines
    # "dotless" is rejected because it has no dot
    assert count == 3
...
def test_file_readers_treat_directories_as_empty(tmp_path: Path) -> None:
    ...
    as_dir.mkdir()
    assert vulnforge.read_lines(as_dir) == []
    assert vulnforge.count_nonblank(as_dir) == 0

    real = tmp_path / "real.txt"
    real.write_text("a.example.com\n")
    dst = tmp_path / "merged.txt"
    # merge should skip the directory and still consume the real file.
    assert vulnforge.merge_unique([as_dir, real], dst) == 1
...
def test_read_jsonl_supports_jsonl_single_object_array_and_bad_input(tmp_path: Path) -> None:
    ...
    assert vulnforge.read_jsonl(jsonl) == [
        {"url": "https://a.example"},
        {"url": "https://b.example"},
    ]
    ...
    assert vulnforge.read_jsonl(obj) == [{"url": "https://single.example"}]

    arr = tmp_path / "arr.json"
    arr.write_text('[{"url":"https://array.example"}]')
    assert vulnforge.read_jsonl(arr) == [{"url": "https://array.example"}]

    bad = tmp_path / "bad.json"
    bad.write_text("{")
    assert vulnforge.read_jsonl(bad) == []
...
def test_fuzzer_json_normalizers(tmp_path: Path) -> None:
    ...
    assert vulnforge._extract_urls_from_ffuf_json(ffuf) == ["200\thttps://x/a"]
...
def test_target_token_normalization(tmp_path: Path) -> None:
    ...
    assert vulnforge._write_target_tokens(src, dst) == 2
...
def test_run_blocking_timeout_writes_log_and_returns_124(tmp_path: Path) -> None:
    ...
    rc, _duration = vulnforge._run_blocking(
...
def test_write_reports_escape_html_and_markdown(tmp_path: Path) -> None:
    ...
    counts = vulnforge._counts(tmp_path)
    state = {"missing_tools": ["x<y"], "tool_failures": {}, "artifacts": {}}

    summary = vulnforge.write_summary(tmp_path, "example.com", state, counts)
    html = vulnforge.write_html(tmp_path, "example.com", counts, ["x<y"])
    md = vulnforge.write_markdown(tmp_path, "example.com", counts, ["x<y"])
...
def test_phase_f2_produces_tls_wp_file(tmp_path: Path) -> None:
    ...
    tools = vulnforge.Tools()

    result = asyncio.run(vulnforge.phase_10_TLSCMS(tmp_path, tools, set(), set()))
...
def test_no_tool_pipeline_generates_reports_without_oast(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    def _no_tools(self: vulnforge.Tools, *names: str) -> list:
        return []
    monkeypatch.setattr(vulnforge.Tools, "have", _no_tools)

    def fail_start(self: vulnforge.Interactsh) -> bool:
        raise AssertionError("OAST should not start for --only 01-RECON,02-RESOLVE")

    monkeypatch.setattr(vulnforge.Interactsh, "start", fail_start)
    ...
    rc = asyncio.run(vulnforge.run_pipeline(args))
...
def test_stages_cover_pipeline_exactly_once() -> None:
    pipeline_names = [name for name, _, _ in vulnforge.PIPELINE]
    staged = [name for stage in vulnforge.STAGES for name in stage]
...
def test_stage_order_respects_dependencies() -> None:
    ...
    stage_of = {name: i for i, stage in enumerate(vulnforge.STAGES) for name in stage}
...
def test_independent_phases_run_concurrently(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    patched = [(name, phase, params) for name, _fn, params in vulnforge.PIPELINE]
    monkeypatch.setattr(vulnforge, "PIPELINE", patched)
    ...
    assert asyncio.run(vulnforge.run_pipeline(args)) == 0
...
def test_cli_help_subprocess() -> None:
    result = subprocess.run(
        [sys.executable, "vulnforge.py", "--help"],
        text=True,
...
def test_phase_g_no_urls_returns_zero(tmp_path: Path) -> None:
    ...
    tools = vulnforge.Tools()
    result = asyncio.run(vulnforge.phase_11_INJECT(tmp_path, tools, set(), set(), None))
    assert result["count"] == 0
...
def test_phase_g_creates_xss_urls_and_ssrf_urls(tmp_path: Path) -> None:
    ...
    tools = vulnforge.Tools()
    asyncio.run(vulnforge.phase_11_INJECT(tmp_path, tools, set(), set(), None))
...
def test_phase_g_skips_when_urls_file_missing(tmp_path: Path) -> None:
    ...
    tools = vulnforge.Tools()
    result = asyncio.run(vulnforge.phase_11_INJECT(tmp_path, tools, set(), set(), "oast.example.com"))
    assert result["count"] == 0
...
def test_phase_g2_no_urls_returns_zero(tmp_path: Path) -> None:
    ...
    result = asyncio.run(vulnforge.phase_12_SSTI(tmp_path, tools, set(), set(), prev))
    assert result["count"] == 0
...
def test_phase_g2_generates_ssti_payloads(tmp_path: Path) -> None:
    ...
    asyncio.run(vulnforge.phase_12_SSTI(tmp_path, tools, set(), set(), prev))
...
def test_mmh3_hash_deterministic() -> None:
    ...
    h1 = vulnforge._mmh3_hash(data)
    h2 = vulnforge._mmh3_hash(b"test favicon data")
    h3 = vulnforge._mmh3_hash(b"different data")
    assert h1 == h2
    assert h1 != h3
...
def test_mmh3_hash_known_value() -> None:
    ...
    h = vulnforge._mmh3_hash(b"hello")
...
def test_phase_j_no_hosts_uses_favicon_fallback(tmp_path: Path) -> None:
    ...
    result = asyncio.run(vulnforge.phase_14_ORIGIN("www.agoda.com", tmp_path, tools, set(), set(), prev))
...
def test_phase_j_crt_subs_parsing(tmp_path: Path) -> None:
    ...
    tools = vulnforge.Tools()
    prev = {"02-RESOLVE": str(tmp_path / "resolved.txt")}
    result = asyncio.run(vulnforge.phase_14_ORIGIN("example.com", tmp_path, tools, set(), set(), prev))
    assert result["count"] >= 0
...
def test_phase_k_no_js_urls_returns_zero(tmp_path: Path) -> None:
    ...
    tools = vulnforge.Tools()
    result = asyncio.run(vulnforge.phase_15_SECRETS(tmp_path, tools, set(), set()))
    assert result["count"] == 0
...
def test_phase_k_regex_patterns_match_expected() -> None:
    ...
    for name, pattern_text in vulnforge._JS_SECRET_PATTERNS:
        for expected_name, sample in test_cases:
            if name == expected_name:
                assert vulnforge._re.search(pattern_text, sample), f"{name} failed to match {sample}"

    # JWT pattern
    jwt_pat = dict(vulnforge._JS_SECRET_PATTERNS)["jwt"]
    assert vulnforge._re.search(jwt_pat, "eyJ0aGlzSXNBZmFrZUFwaVRva2Vu.and0U2Vjb25kUGFydA.dGhpcmRQYXJ0")
    assert not vulnforge._re.search(jwt_pat, "not-a-jwt")
...
def test_phase_k_source_map_regex() -> None:
    ...
    for text, expected in cases:
        m = vulnforge._SOURCE_MAP_RE.search(text)
...
def test_phase_16a_no_endpoints_returns_zero(tmp_path: Path) -> None:
    ...
    tools = vulnforge.Tools()
    result = asyncio.run(vulnforge.phase_16a_AUTHZ(tmp_path, tools, set(), set()))
    assert result["count"] == 0
...
def test_phase_16a_extracts_api_endpoints_from_urls(tmp_path: Path) -> None:
    ...
    tools = vulnforge.Tools()
    asyncio.run(vulnforge.phase_16a_AUTHZ(tmp_path, tools, set(), set()))
...
def test_phase_l_auth_bypass_headers_defined() -> None:
    ...
    assert "X-Original-URL" in vulnforge._AUTH_BYPASS_HEADERS
    assert "X-Forwarded-For" in vulnforge._AUTH_BYPASS_HEADERS
    assert "X-Custom-IP-Authorization" in vulnforge._AUTH_BYPASS_HEADERS
    assert "Authorization: Basic YWRtaW46YWRtaW4=" in vulnforge._AUTH_BYPASS_HEADERS
...
def test_phase_l_mass_assignment_fields_defined() -> None:
    ...
    assert "admin" in vulnforge._MASS_ASSIGN_FIELDS
    assert "is_admin" in vulnforge._MASS_ASSIGN_FIELDS
    assert "role" in vulnforge._MASS_ASSIGN_FIELDS
    assert "balance" in vulnforge._MASS_ASSIGN_FIELDS
...
def test_parse_semver_and_semver_lt() -> None:
    assert vulnforge._parse_semver("1.2.3") == (1, 2, 3)
    assert vulnforge._parse_semver("1.2") == (1, 2, 0)
    assert vulnforge._parse_semver("1") is None
    assert vulnforge._parse_semver("abc") is None
    assert vulnforge._parse_semver("1.2.x") == (1, 2, 0)  # ignores non-digit trailing parts
    assert vulnforge._semver_lt((1, 2, 3), (1, 2, 4))
    assert vulnforge._semver_lt((1, 2, 3), (2, 0, 0))
    assert not vulnforge._semver_lt((2, 0, 0), (1, 9, 9))
    assert not vulnforge._semver_lt((1, 2, 3), (1, 2, 3))
