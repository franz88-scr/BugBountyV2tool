...
class Tools:
    ...
    def have(self, *names: str) -> List[str]:
        ...
        for n in names:
            now = time.monotonic()
            with self._lock:
                if n not in self._cache or (
                    not self._cache[n] and now - self._cache_ts.get(n, 0) > self._RECHECK_INTERVAL
                ):
    ...
    def verify(self, name: str, args: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            if not ok:
                log(
                    "warn",
                    f"tool {name} binary exists but failed verification (rc={result.returncode})",
                )
            return ok
