...
from vulnforge.config import _HOSTNAME_RE
...
def write_findings(path: Path, findings: List[str], phase_id: str = "") -> Dict[str, Any]:
    ...
    log(
        "ok",
        f"{phase_id}: {len(findings)} findings -> {path}"
        if phase_id
        else f"{len(findings)} findings -> {path}",
    )
    if not phase_id:
...
def _load_live_hosts(outdir: Path, domain: str = "") -> List[str]:
    ...
    if not domain:
        ...
        if not domain:
            log(
                "warn",
                "_load_live_hosts: could not auto-detect domain from path; "
...
def _set_proxy_env(proxy: str) -> None:
    from vulnforge.process import _ENV_LOCK
            ...
            try:
                ...
                if mode & 0o077:
                    if fix_permissions:
                        log(
                            "warn",
                            f"cookies.txt has overly permissive permissions ({oct(mode)}); fixing to 0o600",
                        )
                        try:
                        ...
                        log(
                            "warn",
                            f"cookies.txt has overly permissive permissions ({oct(mode)}); consider: chmod 600 {cookie_file}",
                        )
            except OSError:
                pass
            return _sanitize_header_value(
                cookie_file.read_text(encoding="utf-8", errors="ignore").strip()
            )
    else:
            ...
            try:
                ...
                if mode & 0o077:
                    if fix_permissions:
                        log(
                            "warn",
                            f"CWD cookies.txt has overly permissive permissions ({oct(mode)}); fixing to 0o600",
                        )
                        try:
                            cookie_file.chmod(0o600)
                        except OSError:
                        ...
                        log(
                            "warn",
                            f"CWD cookies.txt has overly permissive permissions ({oct(mode)}); consider: chmod 600 {cookie_file}",
                        )
            except OSError:
                pass
            return _sanitize_header_value(
...
def _extra_headers_dict() -> Dict[str, str]:
    ...
    with _CREDENTIAL_LOCK:
        cookie = _CREDENTIAL_STORE.get("COOKIE", "")
    if cookie:
        headers["Cookie"] = _sanitize_header_value(cookie)
    with _CREDENTIAL_LOCK:
        headers_raw = _CREDENTIAL_STORE.get("EXTRA_HEADERS", "")
    if headers_raw:
    ...
    try:
        from vulnforge.process import _PIPELINE_CFG as cfg
...
def _patch_socks(proxy: str) -> bool:
    ...
    with _SOCKS_PATCH_LOCK:
        ...
        try:
            ...
            log(
                "warn",
                "PySocks not installed; SOCKS proxy won't work for Python HTTP. Run: pip install pysocks",
            )
            return False
...
def _unpatch_socks() -> None:
    ...
    with _SOCKS_PATCH_LOCK:
        if _socks_patched and _original_socket_class is not None:
            ...
            _sk.socket = _original_socket_class  # type: ignore[misc]
...
async def _async_urlopen_no_redirect(
    ...
    with _urlopener_cache_lock:
        if (
            _no_redirect_urlopener_cache is None
            or _no_redirect_urlopener_cache_proxy != current_proxy
        ):
...
_throttle_rate_bucket: Optional[Any] = None
_throttle_rate_lock = threading.Lock()
...
async def _throttle_rate() -> None:
    ...
    from vulnforge.process import _PIPELINE_CFG
    from vulnforge.throttle import TokenBucket

    delay = _PIPELINE_CFG.delay
    rate = _PIPELINE_CFG.rate_limit
    if rate > 0:
        global _throttle_rate_bucket
        with _throttle_rate_lock:
            if _throttle_rate_bucket is None or _throttle_rate_bucket.rate != rate:
                _throttle_rate_bucket = TokenBucket(rate, burst=1)
            bucket = _throttle_rate_bucket
        while not bucket.acquire():
            await asyncio.sleep(min(bucket.wait_time(), 0.1))
    elif delay > 0:
        await asyncio.sleep(delay)
...
def _downsample_file(path: Path, n: int = 1) -> None:
    ...
    try:
        ...
        too_many = False
        with (
            path.open(encoding="utf-8", errors="ignore") as src,
...
class Progress:
    def __init__(self, phases: List[str], stages: Optional[List[List[str]]] = None):
        from vulnforge.phases import _PHASE_WEIGHTS
    ...
    def close(self):
        ...
        if self._enabled:
            sys.stderr.write("\033[999;0H\033[K")
            sys.stderr.flush()
        self._last_bar = ""
...
class ScanStatus:
    ...
    _SCAN_STATUS_DIR = (
        ...
        / f"vulnforge_status_{os.getuid()}_{_CONTAINER_ID}"
    )
    ...
    def _flush(self) -> None:
        ...
        with self._write_lock:
            ...
            try:
                ...
                if not os.path.realpath(self._path).startswith(
                    os.path.realpath(self._SCAN_STATUS_DIR).rstrip("/") + "/"
                ):
                    self._path.unlink()
    ...
    def load(cls, domain: str) -> Optional[Dict[str, Any]]:
        ...
        if path.exists():
            try:
                return json.loads(path.read_text(encoding="utf-8", errors="ignore"))  # type: ignore[no-any-return]
