"""MITRE ATT&CK mapping and threat intelligence integration for VulnForge.

Maps scan findings to ATT&CK techniques, provides threat feed queries,
and generates threat intelligence reports.

Usage:
    from vulnforge.threat_intel import map_to_mitre, ThreatIntelEngine
...
class TechniqueMatch:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "sample_findings": [
                f.get("text", f.get("finding", ""))[:120] for f in self.matched_findings[:3]
            ],
        }
                ...
                findings.append(
                    {
                        ...
                        "severity": "high"
                        if art.vuln_type in ("sqli", "xss", "ssrf", "cmdi", "ssti")
                        else "medium",
                    }
                )
    ...
    def generate_report(self, outdir: Path, domain: str = "") -> Path:
        ...
        report = {
            ...
            "summary": {
                ...
                "tactics_covered": list({m.get("tactic", "") for m in mitre_mapping}),
            },
        }
