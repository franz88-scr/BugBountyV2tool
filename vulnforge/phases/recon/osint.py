...
async def phase_84_WHOIS(
    ...
    if creation_match:
        try:
            ...
            if age_months < 6:
                findings.append(
                    f"FLAG: domain registered {age_months:.0f} months ago (< 6 months = suspicious)"
                )
            else:
...
def _add_shodan_auth(req: urllib.request.Request, api_key: str) -> None:
...
async def phase_87_SHODAN(
    ...
    extra_h = _extra_headers_dict()
    try:
        url = f"https://api.shodan.io/dns/domain/{domain}?type=A"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0", **extra_h})
        _add_shodan_auth(req, api_key)
        status, _, body = await _async_urlopen(opener, req, timeout=15)
        if status == 200:
            data = json.loads(body.decode("utf-8", errors="ignore"))
    ...
    try:
        url = f"https://api.shodan.io/shodan/host/search?query=hostname:{domain}"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0", **extra_h})
        _add_shodan_auth(req, api_key)
        status, _, body = await _async_urlopen(opener, req, timeout=15)
        if status == 200:
            ...
            for match in (data.get("matches") or [])[:20]:
                ...
                findings.append(
                    f"[host] {ip}:{port} org={org} product={product} hosts={','.join(hostnames[:3])}"
                )
        else:
...
async def phase_89_PASSIVEDNS(
    ...
    if vt_key:
        try:
            ...
            req = urllib.request.Request(
                url, headers={"x-apikey": vt_key, "User-Agent": "Mozilla/5.0", **extra_h}
            )
            status, _, body = await _async_urlopen(opener, req, timeout=15)
            if status == 200:
                data = json.loads(body.decode("utf-8", errors="ignore"))
    ...
    if st_key:
        try:
            ...
            req = urllib.request.Request(
                url, headers={"apikey": st_key, "User-Agent": "Mozilla/5.0", **extra_h}
            )
            status, _, body = await _async_urlopen(opener, req, timeout=15)
            if status == 200:
                data = json.loads(body.decode("utf-8", errors="ignore"))
