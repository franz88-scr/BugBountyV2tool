...
async def phase_07_PARAMS(
    ...
    if t.has("arjun"):
        ...
        sample_size = (
            min(_PIPELINE_CFG.sample_urls_params, _PIPELINE_CFG.sample_urls_arjun_waf)
            if waf_detected
            else _PIPELINE_CFG.sample_urls_params
        )
        ...
        if arjun_urls:
            ...
            try:
                ...
                if "2.2.7" in _arjun_ver.stdout:
                    log(
                        "warn",
                        "arjun 2.2.7 has a known bug on Python 3.12 (AttributeError: 'dict' object has no attribute 'status_code'); consider pinning to 2.2.6 with: pip install arjun==2.2.6",
                    )
                    _arjun_broken = True
            except Exception:
                pass
            ...
            if waf_detected and sample_size < _PIPELINE_CFG.sample_urls_params:
                log(
                    "info",
                    f"07-PARAMS: WAF detected, reduced arjun sample to {sample_size} URLs with {timeout}s timeout",
                )
    if jobs:
