"""
vulnforge.ai_triage — AI-powered vulnerability triage and severity scoring.
...
from vulnforge.ai import get_provider, parse_json_response
from vulnforge.artifacts import get_findings_for_triage
...
def _chunk_findings(
    findings: List[Dict[str, str]], chunk_size: int = 40
...
def _info_placeholder(f: Dict[str, str]) -> Dict[str, Any]:
    """Info-level placeholder for a finding the LLM could not classify."""
    return {
...
async def _triage_batch(batch: List[Dict[str, str]], domain: str) -> List[Dict[str, Any]]:
    ...
    if isinstance(result, list):
        ...
        for i, f in enumerate(batch):
            ...
            if (
                ...
                and item.get("finding") == f.get("finding")
            ):
                triaged.append(item)
            else:
                triaged.append(_info_placeholder(f))
        return triaged

    return [_info_placeholder(f) for f in batch]
...
async def _generate_summary(
    ...
    combined_low = by_severity["low"] + by_severity["info"]
    ...
    prompt = _SUMMARY_PROMPT_TEMPLATE.format(
        ...
        low_count=len(combined_low),
        low_list=_fmt(combined_low),
    )
...
async def run_triage(
    ...
    if len(all_triaged) != len(findings):
        log(
            "err",
            f"AI Triage: response count mismatch ({len(all_triaged)} vs {len(findings)}), skipping source merge",
        )
        all_triaged = all_triaged[: len(findings)]
        all_triaged += [
            {"severity": "info", "false_positive_probability": 1.0, "justification": "parse error"}
            for _ in range(len(findings) - len(all_triaged))
        ]
    for i in range(len(findings)):
        all_triaged[i]["source"] = findings[i].get("source", "")
        all_triaged[i]["file"] = findings[i].get("file", "")
    ...
    for t in all_triaged:
        ...
        fp_prob = t.get("false_positive_probability", 0)
        if isinstance(fp_prob, (int, float)) and fp_prob > FP_THRESHOLD:
            fp_count += 1
    ...
    fps_list = [
        t
        for t in all_triaged
        if isinstance(t.get("false_positive_probability", 0), (int, float))
        and t["false_positive_probability"] > FP_THRESHOLD
    ]
