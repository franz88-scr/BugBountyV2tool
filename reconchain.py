...
def _get_urlopener() -> Callable[..., Any]:
    """Return a urlopen-compatible callable that respects proxy configuration and SSL.
...
async def _run(name: str, cmd: List[str], timeout: int, outdir: Path, note: str = "") -> StepResult:
    ...
    _cmd_preview = ' '.join(cmd[:6])
    if len(cmd) > 6:
        _cmd_preview += ' …'
    log("info", f"{name}  $ {_cmd_preview}")
...
def _wait_proc(proc: subprocess.Popen, timeout: int) -> bool:
    """Wait for a subprocess, blocking efficiently with the full timeout."""
    try:
...
async def phase_16B_MASSASSIGN(
    ...
    async def _check_mass_assignment(ep: str) -> List[str]:
        results: List[str] = []
        for f in _MASS_ASSIGN_FIELDS[:_PIPELINE_CFG.sample_endpoints_post]:
            await _throttle_rate()
            val = _MASS_ASSIGN_VALUES.get(f, True)
            body = json.dumps({f: val}).encode()
            try:
                ...
                if post_status in (200, 201, 302):
                    results.append(f"  POST {ep} {{{f}: {json.dumps(val)}}} → {post_status}")
            except Exception:
                continue
        return results
        ...
        for f in _MASS_ASSIGN_FIELDS[:_PIPELINE_CFG.sample_endpoints_post]:
            ...
            try:
                ...
                if put_status in (200, 201, 302):
                    results.append(f"  PUT {ep} {{{f}: {json.dumps(val)}}} → {put_status}")
            except Exception:
                continue
        return results
        ...
        for tgt in targets:
            for ep in _GRAPHQL_ENDPOINTS[:5]:  # limit to 5 most common endpoints
                ...
                runner.write_text(
                    ...
                    'timeout 120 clairvoyance "$URL" -o "$OUT"\n'
                )
                runner.chmod(0o755)
    ...
    if t.has("graphinder"):
        ...
        for tgt in targets:
            ...
            runner.write_text(
                ...
                'timeout 120 graphinder --domain "$DOMAIN" --output-file "$OUT"\n'
            )
            runner.chmod(0o755)
            await _run(
        ...
        if gowitness_targets.exists() and read_lines(gowitness_targets):
            ...
            await _run(
                ...
                ["gowitness", "scan", "file", "-f", str(gowitness_targets),
                 "-s", str(screenshots_dir), "--screenshot-format", "png",
                 "--write-db"] + _gw_proxy,
