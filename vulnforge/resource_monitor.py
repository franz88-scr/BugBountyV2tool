...
class AdaptiveSemaphore:
    ...
    def resize(
        self, new_limit: int, *, main_loop: Optional[asyncio.AbstractEventLoop] = None
    ) -> int:
    ...
    def release(self) -> None:
        ...
        with self._lock:
            if self._limit > 0:
                self._permits = min(self._permits + 1, self._limit)
        cond = self._cond
        if cond is None:
            return
        loop = None
        ...
        if loop is not None and loop.is_running():
            ...
            if target_loop is not None and target_loop.is_running():
                ...
                target_loop.call_soon_threadsafe(lambda: target_loop.create_task(_notify_fb()))
            else:
                raise RuntimeError(
                    "AdaptiveSemaphore.release() has no event loop to notify waiters. "
                    "This would cause a permanent deadlock."
                )
...
class AdaptiveThreadSemaphore:
    ...
    def release(self, n: int = 1) -> None:
        with self._lock:
            ...
            allowed = max(0, self._limit - self._count)
            release_n = min(n, allowed)
        if release_n > 0:
            self._sem.release(release_n)
...
class ResourceMonitor:
    ...
    def __init__(
        ...
        ram_emergency_bytes: int = 1 * 1024 * 1024 * 1024,  # 1 GB = PAUSE everything
    ...
    def _count_child_procs() -> int:
        ...
        try:
            ...
            return len(_psutil.Process().children(recursive=True))
    ...
    def _measure_and_adjust(self) -> None:
        ...
        if (
            self.cpu_percent < self._cpu_low
        ...
        # Scale down if ANY pressure signal: high CPU, low RAM, or too many child procs
            ...
            self.cpu_percent > self._cpu_high
