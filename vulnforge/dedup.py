...
class DedupEngine:
    FUZZY_THRESHOLD = 0.995  # Near-identical only — distinct resource IDs must NOT merge
    ...
    _MAX_FUZZY_CANDIDATES = 64  # Cap candidates per fuzzy scan to bound O(n^2) scans
    ...
    def mark_seen(self, key: str, source: str = "", content: str = "") -> None:
        ...
        with self._lock:
            ...
            if len(self._seen) > self.MAX_SEEN:
                ...
                all_keys = [(v.get("ts_epoch", 0.0), k) for k, v in self._seen.items()]
